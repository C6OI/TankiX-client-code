...
namespace Tanks.Lobby.ClientHangar.Impl {
    public class HangarCameraAutoRotateSystem : ECSSystem {
        ...
        public void StartSchedule(NodeAddedEvent e, HangarCameraTankViewStateRotationEnabledNode hangar) {
            ScheduledEvent scheduledEvent = NewEvent<HangarCameraStartAutoRotateEvent>().Attach(hangar).ScheduleDelayed(1f);
            hangar.Entity.AddComponent(new HangarCameraRotateScheduledComponent(scheduledEvent));
        }

        [OnEventFire]
        public void DisableSchedule(NodeRemoveEvent e, HangarCameraTankViewStateRotationEnabledNode nr,
            [JoinSelf] HangarCameraRotateScheduledTankViewStateNode hangar) {
        ...
        [OnEventFire]
        public void DisableAutoRotation(NodeRemoveEvent e, HangarCameraTankViewStateRotationEnabledNode nr,
            [JoinSelf] HangarCameraAutoRotateNode hangar) {
            hangar.hangarCameraRotateScheduled.ScheduledEvent.Manager().Cancel();
        ...
        [OnEventFire]
        public void CheckUserActionOnAnyPointerEvent(EventSystemPointerEvent e,
            SingleNode<ScreenForegroundComponent> foreground,
            [JoinAll] HangarCameraRotateScheduledTankViewStateNode hangar) =>
            ScheduleEvent<HangarCameraDelayAutoRotateEvent>(hangar);

        [OnEventFire]
        public void DelayScheduledEvent(HangarCameraDelayAutoRotateEvent e,
            HangarCameraRotateScheduledTankViewStateNode hangar) {
            hangar.hangarCameraRotateScheduled.ScheduledEvent.Manager().Cancel();

            hangar.hangarCameraRotateScheduled.ScheduledEvent = NewEvent<HangarCameraStartAutoRotateEvent>().Attach(hangar)
                .ScheduleDelayed(hangar.hangarConfig.AutoRotateDelay);
        }

        [OnEventFire]
        public void StopRotate(HangarCameraDelayAutoRotateEvent e, HangarCameraAutoRotateNode hangar) =>
            hangar.Entity.RemoveComponent<HangarCameraAutoRotateComponent>();
