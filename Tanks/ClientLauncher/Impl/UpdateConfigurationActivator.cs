    ...
    public class UpdateConfigurationActivator : UnityAwareActivator<ManuallyCompleting> {
        ...
        void Update() {
            ...
            if (!string.IsNullOrEmpty(wwwLoader.Error)) {
                string errorMessage = string.Format("Update configuration loading was failed. URL: {0}, Error: {1}",
                    wwwLoader.URL,
                    wwwLoader.Error);
            ...
            if (wwwLoader.Bytes == null || wwwLoader.Bytes.Length == 0) {
                HandleError<InvalidLoadedDataErrorEvent>("Empty update configuration data. URL: " + wwwLoader.URL);
                return;
            }
            ...
            try {
                ...
                HandleError<InvalidLoadedDataErrorEvent>(
                    string.Format("Invalid update configuration data. URL: {0}, Error: {1}", wwwLoader.URL, ex.Message),
                    ex);
        ...
        protected override void Activate() {
            ...
            updateConfigUrl = updateConfigUrl.Replace("{DataPath}", Application.dataPath)
                .Replace("{BuildTarget}", BuildTargetName.GetName());
