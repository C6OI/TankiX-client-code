...
namespace Tanks.Battle.ClientCore.Impl {
    public class FlagDropSystem : ECSSystem {
        ...
        [OnEventFire]
        public void FlagDropRequest(UpdateEvent e, CarriedFlagNode flag, [JoinByTank] SelfTankNode tank,
            [JoinByBattle] SingleNode<RoundActiveStateComponent> round,
            [JoinByBattle] SingleNode<CTFConfigComponent> ctfConfig) {
            if (InputManager.GetActionKeyDown(CTFActions.DROP_FLAG)) {
                ...
                NewEvent<TankFlagCollisionEvent>().Attach(flag).Attach(tank)
                    .ScheduleDelayed(ctfConfig.component.enemyFlagActionMinIntervalSec);
            }
        }
