...
namespace Tanks.Battle.ClientCore.Impl {
    public class TankEngineSystem : ECSSystem {
        ...
        public void InitTankEngine(NodeAddedEvent evt, SingleNode<TankEngineConfigComponent> tank) {
            ...
            TankEngineComponent tankEngineComponent = new();
        ...
        [OnEventFire]
        public void RecalculateParametersOnSpeedEffectStart(NodeAddedEvent evt, SpeedEffectNode effect,
            [Context] [JoinByTank] TankEngineNode tank) {
        ...
        [OnEventFire]
        public void RecalculateParametersOnSpeedEffectStop(NodeRemoveEvent evt, SpeedEffectNode effect,
            [JoinByTank] TankEngineNode tank) {
            TankEngineComponent tankEngine = tank.tankEngine;
            TankEngineConfigComponent tankEngineConfig = tank.tankEngineConfig;
        ...
        public void UpdateTankEngine(FixedUpdateEvent evt, TankEngineMovableNode tank) {
            ...
            if (effectiveMoveAxis == 0f) {
                if (effectiveTurnAxis == 0f) {
                    ...
                    UpdateTankEngine(tankEngine,
                        tankEngineConfig,
                        tankEngine.HasValuableCollision,
                        track,
                        evt.DeltaTime,
                        effectiveTurnAxis,
                        tank.speedConfig.TurnAcceleration,
                        tank.speedConfig.ReverseTurnAcceleration,
                        tank.speed.TurnSpeed,
                        tank.tankEngineConfig.EngineTurningBorder);
                }
            } else {
                UpdateTankEngine(tankEngine,
                    tankEngineConfig,
                    ...
                    tank.speedConfig.ReverseAcceleration,
                    tank.speed.Speed,
                    tankEngine.MovingBorder);
            }
        }

        void UpdateTankEngine(TankEngineComponent tankEngine, TankEngineConfigComponent tankEngineConfig, bool hasCollision,
            TrackComponent track, float dt, float currentAxis, float straightAcceleration, float reverseAcceleration,
            float maxSpeed, float border) {
