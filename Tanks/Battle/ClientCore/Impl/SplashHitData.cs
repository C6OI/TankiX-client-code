...
namespace Tanks.Battle.ClientCore.Impl {
    public class SplashHitData {
        ...
        const string LOG_FORMAT =
            "Splash Hit Data: direct targets = [{0}] ; static hit = {1} ; splash center = {2} splash targets = [{3}]";
        ...
        public Vector3 SplashCenter {
            get => splashCenter;
            set => splashCenter = value;
        }

        public Entity ExcludedEntityForSplashHit { get; set; }

        public static SplashHitData CreateSplashHitData(List<HitTarget> directTargets, StaticHit staticHit,
            Entity weaponHitEntity) {
        ...
        public override string ToString() => string.Format(
            "Splash Hit Data: direct targets = [{0}] ; static hit = {1} ; splash center = {2} splash targets = [{3}]",
            ConvertTargetsCollectionToString(DirectTargets),
            StaticHit != null ? StaticHit.ToString() : string.Empty,
            splashCenter.ToString(),
            ConvertTargetsCollectionToString(SplashTargets));

        string ConvertTargetsCollectionToString(List<HitTarget> targets) =>
            string.Join(", ", targets.Select(i => i.ToString()).ToArray());
    }
}