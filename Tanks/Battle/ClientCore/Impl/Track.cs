...
namespace Tanks.Battle.ClientCore.Impl {
    public class Track {
        ...
        public Track(Rigidbody rigidbody, int numRays, Vector3 trackCenterPosition, float trackLength,
            ChassisConfigComponent chassisConfig, ChassisComponent chassis, int side, float damping) {
            this.side = side;
        ...
        public void UpdateSuspensionContacts(float dt) {
            numContacts = 0;
        ...
        void CreateSuspensionRays(Rigidbody rigidbody, int numRays, Vector3 trackCenterPosition, float trackLength,
            ChassisConfigComponent chassisConfig, ChassisComponent chassis, float damping) {
            rays = new SuspensionRay[numRays];
            float num = trackLength / (rays.Length - 1);

            for (int i = 0; i < numRays; i++) {
                Vector3 origin = new(trackCenterPosition.x,
                    trackCenterPosition.y,
                    trackCenterPosition.z + 0.5f * trackLength - i * num);
