...
namespace Tanks.Battle.ClientCore.Impl {
    public class SectorDirectionsCollectorSystem : AbstractDirectionsCollectorSystem {
        [OnEventFire]
        public void CollectSectorDirectionsEvent(CollectSectorDirectionsEvent evt, VerticalTargetingNode verticalTargeting) {
            ...
            if (evt.TargetSectors.Count != 0) {
                Vector3 leftDirectionWorld =
                    new MuzzleLogicAccessor(verticalTargeting.muzzlePoint, verticalTargeting.weaponInstance)
                        .GetLeftDirectionWorld();

                TargetSectorUtils.CutSectorsByOverlap((LinkedList<TargetSector>)evt.TargetSectors,
                    1f / verticalSectorsTargeting.RaysPerDegree);
        ...
        void CollectSectorDirections(TargetingData targetingData, Vector3 rotationAxis, TargetSector targetSector,
            int numRays) {
            numRays = numRays / 2 * 2 + 1;
            float num = (targetSector.Up + targetSector.Down) * 0.5f;
            float num2 = targetSector.Length() / (numRays - 1);
            float num3 = 0f;

            for (int i = 0; i < numRays; i++) {
                Vector3 dir = Quaternion.AngleAxis(num + num3, rotationAxis) * targetingData.Dir;
                CollectDirection(targetingData.Origin, dir, Mathf.Abs(num + num3), targetingData);
