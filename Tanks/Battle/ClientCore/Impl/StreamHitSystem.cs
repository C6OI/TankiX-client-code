    ...
    public class StreamHitSystem : ECSSystem {
        ...
        public void ValidateHitEvent(SelfHitEvent e, HitNode weapon) {
            if (e.Targets != null && e.Targets.Count > 0) {
                ...
                if (entity != entity2) {
                    throw new Exception(string.Concat("Invalid stream hit. targetTankInEvent=",
                        entity,
                        " targetTankInComponent=",
                        entity2));
                }
            }
        }
        ...
        [OnEventFire]
        public void UpdateExistenceBeforeHit(SendHitToServerEvent e, CheckingNode weapon) {
            Log.Debug("Update existence before hit");
            UpdateHitExistence(e.TargetingData, weapon);
        }

        [OnEventFire]
        public void UpdateDataBeforeHit(SendHitToServerEvent e, HitNode weapon) {
            if (HasHit(e.TargetingData, weapon.streamHitConfig)) {
                Log.Debug("Update data before hit");
        ...
        public void Check(UpdateEvent e, CheckingNode weapon) {
            if (weapon.Entity.HasComponent<StreamHitComponent>() ||
                !(weapon.streamHitChecking.LastCheckTime + weapon.streamHitConfig.LocalCheckPeriod > UnityTime.time)) {
        ...
        [OnEventFire]
        public void UpdateChecking(StreamHitResultEvent e, CheckingNode weapon) =>
            UpdateHitExistence(e.TargetingData, weapon);
        ...
        bool HasHit(TargetingData targetingData, StreamHitConfigComponent streamHitConfigComponent) =>
            targetingData.BestDirection.HasTargetHit() ||
            streamHitConfigComponent.DetectStaticHit && targetingData.BestDirection.HasStaticHit();

        [OnEventComplete]
        public void UpdateHit(StreamHitResultEvent e, HitNode weapon) {
            Log.Debug("UpdatePeriodic");
        ...
        void UpdateHitData(HitNode weapon, TargetingData targetingData, bool skipTimeoutCheck) {
            ...
            if (!IsSameTank(tankHit, streamHitChecking.LastSentTankHit)) {
                flag = true;
            } else if (skipTimeoutCheck ||
                       streamHitChecking.LastSendToServerTime + streamHitConfig.SendToServerPeriod < UnityTime.time) {
            ...
            if (flag) {
                ...
                Log.Info("Send to server: update");
                ScheduleEvent(new SelfUpdateStreamHitEvent(tankHit, staticHit), weapon);
                SaveHitSentToServer(streamHitChecking, streamHit);
            }
        }

        void AddStreamHit(Entity weapon, TargetingData targetingData, StreamHitConfigComponent config,
            StreamHitCheckingComponent checking) {
        ...
        bool IsAlmostEqual(HitTarget tankHit, HitTarget lastSentTankHit) =>
            tankHit == null ||
            MathUtil.NearlyEqual(tankHit.LocalHitPoint, lastSentTankHit.LocalHitPoint, NEAR_HIT_POSITION_EPSILON);
        ...
        static HitTarget GetTankHit(TargetingData targetingData) =>
            !targetingData.BestDirection.HasTargetHit() ? null
                : HitTargetAdapter.Adapt(targetingData.BestDirection.Targets.Single());
