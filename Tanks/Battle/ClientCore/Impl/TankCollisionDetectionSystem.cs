...
namespace Tanks.Battle.ClientCore.Impl {
    public class TankCollisionDetectionSystem : ECSSystem {
        ...
        public void EnableDetector(NodeAddedEvent nodeAdded, TankNode tank) {
            ...
            boundsCollider.gameObject.AddComponent<Rigidbody>().isKinematic = true;
        ...
        [OnEventFire]
        public void DisableDetector(NodeRemoveEvent nodeRemoveEvent, TankCollisionDetectionForNRNode nr,
            [JoinSelf] TankCollisionDetectionNode tank) {
            BoxCollider boundsCollider = tank.tankColliders.BoundsCollider;
            tank.Entity.RemoveComponent<TankCollisionDetectionComponent>();
            Object.DestroyImmediate(tank.tankCollisionDetection.Detector);
            Object.DestroyImmediate(boundsCollider.gameObject.GetComponent<Rigidbody>());
            boundsCollider.isTrigger = false;
        }

        [OnEventFire]
        public void PrepareAndSendCollisionResult(FixedUpdateEvent e, TankCollisionDetectionNode tank,
            [JoinByBattle] BattleTankCollisionsNode battle) {
            ...
            if (tankCollisionDetection.Detector.UpdatesCout >= DETECTOR_TRUST_UPDATES_COUNT) {
                ...
                if (tankCollisionDetection.Phase < semiActiveCollisionsPhase &&
                    tankCollisionDetection.Detector.CanBeActivated) {
