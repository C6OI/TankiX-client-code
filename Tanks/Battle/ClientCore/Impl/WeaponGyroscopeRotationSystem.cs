...
namespace Tanks.Battle.ClientCore.Impl {
    public class WeaponGyroscopeRotationSystem : ECSSystem {
        [OnEventFire]
        public void TakeOrientationWeapon(NodeAddedEvent evt, WeaponGyroscopeNode weaponGyroscope,
            [JoinByTank] [Context] TankNode tank) => UpdateGyroscopeData(weaponGyroscope, tank);

        [OnEventComplete]
        public void RotateWeapon(WeaponRotationUpdateGyroscopeEvent e, WeaponGyroscopeNode weapon,
            [JoinByTank] TankNode tank, [JoinByTank] Optional<SingleNode<VulcanWeaponComponent>> vulkanWeapon,
            [JoinAll] SelfBattleUser selfBattleUser) {
            ...
            weaponGyroscopeRotation.DeltaAngleOfHullRotation =
                CalculateDeltaAngleOfHullRotationAroundUpAxis(forwardDir, forwardDir2, upDir, upDir2);

            float num = tank.speed.TurnSpeed * e.DeltaTime;

            weaponGyroscopeRotation.DeltaAngleOfHullRotation =
                Mathf.Clamp(weaponGyroscopeRotation.DeltaAngleOfHullRotation, 0f - num, num);
            ...
            if (weaponRotationControl.ForceGyroscopeEnabled) {
                if (mouseControlEnable) {
                    weaponRotationControl.MouseRotationCumulativeHorizontalAngle -=
                        weaponGyroscopeRotation.DeltaAngleOfHullRotation * (1f - gyroscopePower);
                }

                float weaponTurnDecelerationCoeff = vulkanWeapon.Get().component.WeaponTurnDecelerationCoeff;

                weaponGyroscopeRotation.WeaponTurnCoeff =
                    weaponTurnDecelerationCoeff + (1f - gyroscopePower) * (1f - weaponTurnDecelerationCoeff);

                weaponGyroscopeRotation.DeltaAngleOfHullRotation *= gyroscopePower;
            }
        }

        void UpdateGyroscopeData(WeaponGyroscopeNode weaponGyroscope, TankNode tank) {
        ...
        float CalculateDeltaAngleOfHullRotationAroundUpAxis(Vector3 prevForward, Vector3 nextForward, Vector3 prevUp,
            Vector3 nextUp) {
