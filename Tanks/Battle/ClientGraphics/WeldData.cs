...
namespace Tanks.Battle.ClientGraphics {
    public class WeldData {
        ...
        long GetWeldHash(Vector3 position) => (long)(position.x * percision) % dimension +
                                              (long)(position.y * percision) % dimension * dimension +
                                              (long)(position.z * percision) % dimension * dimension * dimension;
    }
}