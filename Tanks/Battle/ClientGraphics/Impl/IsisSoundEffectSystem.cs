...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class IsisSoundEffectSystem : ECSSystem {
        [OnEventFire]
        public void InitIsisSounds(NodeAddedEvent evt, [Combine] InitialIsisSoundEffectNode weapon,
            SingleNode<SoundListenerBattleStateComponent> soundListener) => InitIsisSounds(weapon);
        ...
        [OnEventFire]
        public void EnableIsisTargetSound(NodeAddedEvent e, TargetEffectNode weapon,
            [JoinByTank] [Context] IsisCurrentSoundEffectNode isisSound, [Context] [JoinByBattle] DMNode dm) {
        ...
        [OnEventFire]
        public void EnableIsisTargetSound(NodeAddedEvent e, [Combine] TargetEffectTeamNode weapon,
            [JoinByTank] [Context] [Combine] IsisCurrentSoundEffectNode isisSound,
            [JoinByTeam] [Context] TeamNode weaponTeam) {
            Entity entity = weapon.streamHit.TankHit.Entity;
            NewEvent<UpdateIsisSoundModeEvent>().Attach(weapon).Attach(entity).Schedule();
        }

        [OnEventFire]
        public void UpdateIsisSoundMode(UpdateIsisSoundModeEvent evt, IsisCurrentSoundTeamEffectNode weapon,
            TankTeamNode target) {
            SoundController soundController = weapon.teamGroup.Key != target.teamGroup.Key
                                                  ? weapon.isisDamagingSoundEffect.SoundController
                                                  : weapon.isisHealingSoundEffect.SoundController;

            UpdateIsisSoundEffect(weapon.isisCurrentSoundEffect, soundController);
        }

        [OnEventFire]
        public void DisableIsisTargetSound(NodeRemoveEvent e, TargetEffectNode weapon,
            [JoinByTank] IsisCurrentSoundEffectNode isisSound) {
            IsisCurrentSoundEffectComponent isisCurrentSoundEffect = isisSound.isisCurrentSoundEffect;
            SoundController soundController = weapon.isisHealingSoundEffect.SoundController;
            UpdateIsisSoundEffect(isisCurrentSoundEffect, soundController);
        }
