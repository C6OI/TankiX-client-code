...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class DecalPolygonCollector {
        readonly MeshBuffersCache cache;

        public DecalPolygonCollector(MeshBuffersCache meshBuffersCache) => cache = meshBuffersCache;
        ...
        public bool Collect(DecalProjection projection, MeshBuilder meshBuilder) {
            ...
            foreach (Collider collider in colliders) {
                ...
                if (!TryGetRenderer(collider.gameObject, out renderer)) {
                    continue;
                }
                ...
                for (int j = 0; j < triangles.Length; j += 3) {
                    ...
                    if (!(sqrMagnitude > num2 * num2)) {
                        ...
                        if (!(num4 < 0f)) {
                            ...
                            num3 = meshBuilder.AddVertexWeldAndTransform(num3,
                                new VertexData(vertex, normal, surfaceType),
                                transform);

                            num5 = meshBuilder.AddVertexWeldAndTransform(num5,
                                new VertexData(vertex2, normal2, surfaceType2),
                                transform);

                            num6 = meshBuilder.AddVertexWeldAndTransform(num6,
                                new VertexData(vertex3, normal3, surfaceType3),
                                transform);
        ...
        bool TryGetRenderer(GameObject gameObject, out Renderer renderer) {
            renderer = null;
        ...
        Collider[] GetColliders(DecalProjection projection) =>
            Physics.OverlapSphere(projection.ProjectionHit.point, projection.HalfSize);

        bool TriangleSphereIntersectSphere(Vector3 triangleV0, Vector3 triangleV1, Vector3 triangleV2,
            Vector3 spherePosition, float radius) {
            bool flag = false;
            ...
            return (vector - triangleV0).sqrMagnitude > num2 ||
                   (vector - triangleV1).sqrMagnitude > num2 ||
                   (vector - triangleV2).sqrMagnitude > num2;
        }

        bool GetMeshByCollider(Collider collider, out Mesh mesh) {
            mesh = null;
