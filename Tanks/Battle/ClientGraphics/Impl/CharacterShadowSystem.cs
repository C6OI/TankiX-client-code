...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class CharacterShadowSystem : ECSSystem {
        readonly List<CharacterShadowNode> visibleCharacters = new();

        [OnEventFire]
        public void Init(NodeAddedEvent evt, NotInitedCharacterShadowNode node,
            [JoinAll] SingleNode<CharacterShadowCommonSettingsComponent> characterShadowCommonSettings) {
            if (!node.characterShadowCasters.HasBounds) {
                LoggerProvider.GetLogger(this).Error("Incorrect character shadow elements on gameObject " +
                                                     node.characterShadow.gameObject.name);

                return;
            }

            CharacterShadowInternalComponent characterShadowInternalComponent = new();

            CalculateShadowInternalData(characterShadowInternalComponent,
                node.characterShadow,
                characterShadowCommonSettings.component);
        ...
        [OnEventFire]
        public void UpdateFadeAlpha(TimeUpdateEvent e, FadeShadowNode fadeNode,
            [JoinByTank] SingleNode<TankDeadStateComponent> deadTank,
            [JoinAll] SingleNode<CharacterShadowCommonSettingsComponent> shadowHelper) =>
            fadeNode.characterShadow.color.a =
                fadeNode.transparencyTransition.CurrentAlpha * fadeNode.characterShadowInternal.BaseAlpha;

        [OnEventComplete]
        public void UpdateCharacterShadows(TimeUpdateEvent evt,
            SingleNode<CharacterShadowCommonSettingsComponent> characterShadowCommonSettingsNode,
            [JoinAll] ICollection<CharacterShadowNode> characters, [JoinAll] BattleCameraNode battleCamera) {
            CharacterShadowCommonSettingsComponent component = characterShadowCommonSettingsNode.component;
            UpdateBoundsAndCullCharacters(visibleCharacters, characters, battleCamera.camera.Camera, component);
        ...
        void CalculateShadowInternalData(CharacterShadowInternalComponent shadowInternalData,
            CharacterShadowComponent characterShadow, CharacterShadowCommonSettingsComponent shadowCommonSettings) {
        ...
        Projector CreateProjector(CharacterShadowCommonSettingsComponent shadowCommonSettings) {
            GameObject gameObject = new("CharacterShadowProjector");
        ...
        void UpdateBoundsAndCullCharacters(ICollection<CharacterShadowNode> collector,
            ICollection<CharacterShadowNode> characters, Camera camera, CharacterShadowCommonSettingsComponent settings) {
            ...
            foreach (CharacterShadowNode character in characters) {
                ...
                Bounds boundsInLightSpace = BoundsUtils.TransformBounds(character.characterShadowCasters.BoundsInWorldSpace,
                    worldToLocalMatrix);
                ...
                characterShadowInternal.ProjectionBoundInLightSpace =
                    CalculateProjectionBoundInLightSpace(settings, boundsInLightSpace, characterShadow.offset);
        ...
        void CalculateProjectionData(CharacterShadowCommonSettingsComponent settings,
            ICollection<CharacterShadowNode> characters, Camera camera) {
            ...
            foreach (CharacterShadowNode character in characters) {
                ...
                SetProjectorData(characterShadowInternal,
                    localToWorldMatrix.MultiplyPoint3x4(projectionBoundInLightSpace.center),
                    rotation,
                ...
                float left = (-num5 * 2 - 1) * num6;
                float right = (-num5 * 2 - 1 + num * 2) * num6;
                float bottom = (-num4 * 2 - 1) * num6;
                float top = (-num4 * 2 - 1 + num2 * 2) * num6;
                Matrix4x4 matrix4x3 = Matrix4x4.Ortho(left, right, bottom, top, 0f - z, z);

                characterShadowInternal.CasterMaterial.SetMatrix("ViewToAtlas",
                    GL.GetGPUProjectionMatrix(matrix4x3 * matrix4x2, true));

                num3++;
            }
        }

        static Rect CalculateLocationInAtlas(int countX, int countY, int xIndex, int yIndex) {
            float num = 1f / countX;
            float num2 = 1f / countY;
            float x = 2 * xIndex * num * 0.5f;
            float y = 2 * yIndex * num2 * 0.5f;
            return new Rect(new Rect(new Vector2(x, y), new Vector2(num, num2)));
        }

        Bounds CalculateProjectionBoundInLightSpace(CharacterShadowCommonSettingsComponent settings,
            Bounds boundsInLightSpace, float offset) {
            ...
            result.center = new Vector3(boundsInLightSpace.center.x,
                boundsInLightSpace.center.y,
                boundsInLightSpace.max.z - result.extents.z);

            return result;
        }

        void SetProjectorData(CharacterShadowInternalComponent characterInternal, Vector3 position, Quaternion rotation,
            CharacterShadowComponent characterShadow, Rect atlasData) {
            ...
            projector.orthographicSize =
                Mathf.Max(projectionBoundInLightSpace.extents.x, projectionBoundInLightSpace.extents.y);
        ...
        void GenerateDrawCommandBuffer(CharacterShadowCommonSettingsComponent settings,
            ICollection<CharacterShadowNode> characters) {
            RenderTexture shadowMap = settings.shadowMap;
            CommandBuffer commandBuffer = settings.CommandBuffer;
            commandBuffer.Clear();

            commandBuffer.GetTemporaryRT(settings.RawShadowMapNameId,
                shadowMap.width,
                shadowMap.height,
                16,
                FilterMode.Point,
            ...
            foreach (CharacterShadowNode character in characters) {
                GenerateDrawCharacterCommands(commandBuffer,
                    character.characterShadowCasters,
                    character.characterShadowInternal);
            }

            commandBuffer.GetTemporaryRT(settings.BlurredShadowMapNameId,
                shadowMap.width,
                shadowMap.height,
                0,
                FilterMode.Point,
            ...
            commandBuffer.Blit(settings.RawShadowMapId,
                settings.BlurredShadowMapId,
                settings.HorizontalBlurMaterial,
                settings.blurSize);

            commandBuffer.ReleaseTemporaryRT(settings.RawShadowMapNameId);

            commandBuffer.Blit(settings.BlurredShadowMapId,
                new RenderTargetIdentifier(shadowMap),
                settings.VerticalBlurMaterial,
                settings.blurSize);

            commandBuffer.ReleaseTemporaryRT(settings.BlurredShadowMapNameId);
        }

        void GenerateDrawCharacterCommands(CommandBuffer commandBuffer, CharacterShadowCastersComponent casters,
            CharacterShadowInternalComponent shadowInternal) {
        ...
        void UpdateShadowMapSize(CharacterShadowCommonSettingsComponent settings,
            ICollection<CharacterShadowNode> visibleCharacters, ICollection<CharacterShadowNode> allCharacters) {
            ...
            if (num3 > 0) {
                settings.shadowMap = new RenderTexture(num3,
                    num3,
                    0,
                    settings.ShadowMapTextureFormat,
                    RenderTextureReadWrite.Linear);
