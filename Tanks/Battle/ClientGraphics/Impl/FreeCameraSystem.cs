...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class FreeCameraSystem : ECSSystem {
        static readonly Vector3[] clipDirections = new Vector3[6] {
            Vector3.right,
            Vector3.left,
            Vector3.up,
            Vector3.down,
        ...
        static readonly float CLIP_DISTANCE = 2f;
        ...
        public void RecieveInput(TimeUpdateEvent evt, FreeCameraNode cameraNode, [JoinAll] UserNode user) {
            ...
            float num3 = ((InputManager.CheckAction(FreeCameraActions.UP_TURN) ? -1 : 0) +
                          (InputManager.CheckAction(FreeCameraActions.DOWN_TURN) ? 1 : 0)) *
                         0.5f;

            float num4 = ((InputManager.CheckAction(FreeCameraActions.RIGHT_TURN) ? 1 : 0) +
                          (InputManager.CheckAction(FreeCameraActions.LEFT_TURN) ? -1 : 0)) *
                         0.5f;

            if (InputManager.CheckMouseButtonInAllActiveContexts(FreeCameraActions.MOUSE_BUTTON_DOWN,
                    UnityInputConstants.MOUSE_BUTTON_LEFT)) {
            ...
            float num5 = (InputManager.CheckAction(FreeCameraActions.FORWARD_MOVING) ? 1 : 0) +
                         (InputManager.CheckAction(FreeCameraActions.BACK_MOVING) ? -1 : 0);

            float num6 = (InputManager.CheckAction(FreeCameraActions.RIGHT_MOVING) ? 1 : 0) +
                         (InputManager.CheckAction(FreeCameraActions.LEFT_MOVING) ? -1 : 0);

            float num7 = InputManager.GetAxis(FreeCameraActions.MOUSE_SCROLL_WHEEL_UP, true) -
                         InputManager.GetAxis(FreeCameraActions.MOUSE_SCROLL_WHEEL_DOWN, true);
        ...
        static Vector3 NormalizeEuler(Vector3 euler, float xMinAngle, float xMaxAngle) {
        ...
        static Vector3 GetClippedPosition(Vector3 currentPosition, Vector3 targetPosition) {
            RaycastHit hitInfo;

            for (int i = 0; i < clipDirections.Length; i++) {
                Vector3 vector = targetPosition - currentPosition;

                if (Vector3.Dot(vector, clipDirections[i]) > 0f &&
                    Physics.Linecast(currentPosition - clipDirections[i],
                        currentPosition + clipDirections[i] * CLIP_DISTANCE,
                        out hitInfo,
                        LayerMasks.STATIC)) {
            ...
            if (Physics.Linecast(targetPosition + Vector3.up,
                    targetPosition + Vector3.down * CLIP_DISTANCE,
                    out hitInfo,
                    LayerMasks.STATIC)) {
