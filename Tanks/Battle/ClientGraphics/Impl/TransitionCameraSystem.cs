...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class TransitionCameraSystem : ECSSystem {
        ...
        static readonly float SPECTATOR_TRANSITION_CAMERA_POSITION_SMOOTHING_RATIO = 10f;

        static readonly float SPECTATOR_TRANSITION_CAMERA_ROTATION_SMOOTHING_RATIO = 12f;

        [OnEventFire]
        public void InitCamera(NodeAddedEvent evt, TransitionCameraNode transitionCameraNode, TankNode tank,
            [Context] [JoinByTank] WeaponNode weapon, [JoinByUser] SingleNode<SelfTankComponent> selfTank) {
            SpawnCameraConfigUnityComponent spawnCameraConfigUnity = transitionCameraNode.spawnCameraConfigUnity;

            InitFollowCamera(transitionCameraNode,
                weapon,
                tank,
                spawnCameraConfigUnity.FlyTimeSec,
                spawnCameraConfigUnity.FlyHeight);
        }

        [OnEventFire]
        public void InitCamera(NodeAddedEvent evt, SpectatorTransitionCameraNode transitionCameraNode, TankNode tank,
            [JoinByTank] [Context] WeaponNode weapon, [JoinByUser] FollowedUserNode followedUser) {
            float num = 0.55f;
            float num2 = 1f;
            ...
            if (transitionCameraNode.transitionCamera.Spawn) {
                ...
                TransitionCameraConfigUnityComponent transitionCameraConfigUnity =
            ...
            switch (transitionCameraNode.transitionCamera.CameraSaveData.Type) {
                ...
                case CameraType.MouseOrbit: {
                    TransformData targetMouseOrbitCameraTransformData =
                        MouseOrbitCameraUtils.GetTargetMouseOrbitCameraTransformData(
                            weapon.cameraTarget.TargetObject.transform,
                            transitionCameraNode.transitionCamera.CameraSaveData.MouseOrbitDistance,
                            transitionCameraNode.transitionCamera.CameraSaveData.MouseOrbitTargetRotation);

                    InitCamera(transitionCameraNode,
                        targetMouseOrbitCameraTransformData.Position,
                        targetMouseOrbitCameraTransformData.Rotation.eulerAngles,
                        num,
                        num2);
        ...
        void InitFollowCamera(TransitionCameraNode cameraNode, WeaponNode weapon, TankNode tank, float flyTimeSec,
            float flyHeight) {
            ...
            Vector3 position = CameraPositionCalculator.CalculateCameraPosition(transform,
                tank.baseRenderer,
        ...
        public void InitCamera(NodeAddedEvent evt, SpectatorTransitionCameraNode transitionCameraNode) {
            ...
            if (type == CameraType.Free) {
                ...
                InitCamera(transitionCameraNode,
                    position,
                    eulerAngles,
                    transitionCameraConfigUnity.FlyTimeSec,
                    transitionCameraConfigUnity.FlyHeight);
            }
        }

        static void InitCamera(TransitionCameraNode transitionCameraNode, Vector3 position, Vector3 angles, float flyTimeSec,
            float flyHeight) {
            ...
            float y = (!(transitionCamera.P1.y > transitionCamera.P4.y) ? transitionCamera.P4.y : transitionCamera.P1.y) +
                      flyHeight;
        ...
        public void UpdateCamera(TimeUpdateEvent e, TransitionCameraNode transitionCameraNode,
            [JoinAll] SingleNode<SelfTankComponent> selfTank) {
        ...
        [OnEventFire]
        public void UpdateCamera(TimeUpdateEvent e, SpectatorTransitionCameraNode transitionCameraNode,
            [JoinAll] Optional<SingleNode<CameraTargetComponent>> target) {
            ...
            if (transitionCameraNode.transitionCamera.TransitionComplete) {
                switch (cameraSaveData.Type) {
                    case CameraType.Follow:
                        transitionCameraNode.bezierPosition.BezierPosition.SetBaseRatio(cameraSaveData
                            .FollowCameraBezierPositionRatio);

                        transitionCameraNode.bezierPosition.BezierPosition.SetRatioOffset(cameraSaveData
                            .FollowCameraBezierPositionRatioOffset);

                        transitionCameraNode.cameraESM.Esm.ChangeState<CameraStates.CameraFollowState>();
                        break;

                    case CameraType.MouseOrbit: {
                        MouseOrbitCameraComponent mouseOrbitCamera = transitionCameraNode.cameraESM.Esm
                            .ChangeState<CameraStates.CameraOrbitState>().mouseOrbitCamera;
        ...
        static void UpdateCamera(float dt, TransitionCameraNode cameraNode) {
            TransitionCameraComponent transitionCamera = cameraNode.transitionCamera;
            ...
            Vector3 position = Bezier.PointOnCurve(transitionCamera.Distance / transitionCamera.TotalDistance,
                transitionCamera.P1,
                transitionCamera.P2,
                transitionCamera.P3,
                transitionCamera.P4);
            ...
            Vector3 euler = eulerAngles +
                            new Vector3(transitionCamera.AngleValuesX.Update(dt),
                                transitionCamera.AngleValuesY.Update(dt),
                                0f);
