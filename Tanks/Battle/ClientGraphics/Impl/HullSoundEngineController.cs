...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class HullSoundEngineController : MonoBehaviour {
        ...
        [SerializeField] float acelerationRPM = 1.5f;

        [SerializeField] float decelerationRPM = 1.5f;

        [SerializeField] [Range(0f, 1f)] float increasingLoadThreshold = 1f / 35f;

        [Range(0f, 1f)] [SerializeField] float decreasingLoadThreshold = 1f / 35f;
        ...
        [SerializeField] int RPMDataArrayLength;

        [SerializeField] int lastRPMDataIndex;

        [Range(0f, 1f)] [SerializeField] float inputRPMFactor;
        ...
        bool IsStableWork {
            ...
            set {
                if (isStableWork != value) {
                    if (value) {
                        t = 0f;
                        hesitationShockIntervalTimer = GetShockInterval();
                    }
            ...
            set {
                ...
                if (IsStableWork) {
                    if (hesitationShockIntervalTimer > 0f) {
                        hesitationLevel = Mathf.Sin(hesitationFrequency * t);
                    } else {
                        hesitationLevel = !(hesitationLevel >= 0f) ? Random.Range(0.5f, 1f) : Random.Range(-1f, -0.5f);
                        t = Mathf.Asin(hesitationLevel) / hesitationFrequency;
                        hesitationShockIntervalTimer = GetShockInterval();
                    }
                } else {
                    hesitationLevel = 0f;
        ...
        float GetShockInterval() => Random.Range(hesitationShockMinInterval, hesitationShockMaxInterval);
        ...
        void UpdateRealNRMFactor(float dt) {
            ...
            if (destroyTimer > 0f) {
                ...
                float num2 = !(num > 0f) ? decelerationRPM : acelerationRPM;
        ...
        void UpdateEngineLoad(float dt) {
            ...
            if (num != EngineLoad) {
                float num8 = EngineLoad;

                if (num - EngineLoad > 0f) {
                    num8 += increasingLoadSpeed * dt;
                    num8 = Mathf.Min(num8, num);
                } else {
                    num8 -= decreasingLoadSpeed * dt;
                    num8 = Mathf.Max(num8, num);
                }

                EngineLoad = Mathf.Clamp01(num8);
            }
        }
        ...
        public void Build() {
            ...
            for (int i = 0; i < RPMDataArrayLength; i++) {
                float prevRPM = i != 0 ? RPMSoundBehaviourArray[i - 1].RPM
                                    : minRPM - (extremalRPMStartOffset + hesitationAmplitudeRPM) / blendRange;

                float nextRPM = i != lastRPMDataIndex ? RPMSoundBehaviourArray[i + 1].RPM
                                    : maxRPM + (extremalRPMEndOffset + hesitationAmplitudeRPM) / blendRange;
        ...
        public bool IsRPMWithinRange(RPMSoundBehaviour rpmSoundBehaviour, float rpm) =>
            IsRPMAboveBeginRange(rpm, rpmSoundBehaviour) && IsRPMBelowEndRange(rpm, rpmSoundBehaviour);
    }
}