...
namespace Tanks.Battle.ClientHUD.Impl {
    public class PauseAndIdleKickHUDSystem : ECSSystem {
        ...
        [OnEventFire]
        public void ShowPauseServiceMessage(NodeAddedEvent e, PausedBattleUserNode pausedUser,
            [JoinAll] PauseServiceMessageNode serviceMessage) => serviceMessage.serviceMessageESM.Esm
            .ChangeState<ServiceMessageStates.ServiceMessageVisibleState>();

        [OnEventFire]
        public void HidePauseServiceMessage(NodeRemoveEvent e, PausedBattleUserNode pausedUser,
            [JoinAll] PauseServiceMessageNode serviceMessage) => serviceMessage.serviceMessageESM.Esm
        ...
        [OnEventFire]
        public void CheckAutokickTime(CheckAutokickTimeEvent e, BattleUserCounterWithWarningNode battleUser,
            [JoinAll] AutokickServiceMessageNode autokickServiceMessage,
            [JoinAll] HiddenPauseServiceMessageNode serviceMessage) {
            float num = IdleKickUtils.CalculateTimeLeft(battleUser.idleCounter,
                battleUser.idleBeginTime,
                battleUser.idleKickConfig);
        ...
        [OnEventFire]
        public void UpdateGUI(UpdateEvent e, BattleUserNode battleUser,
            [JoinByUser] Optional<BattleUserCounterNode> idleCounter,
            [JoinBy(typeof(BattleGroupComponent))] ActiveRoundNode activeRound,
            [JoinAll] VisiblePauseServiceMessageNode serviceMessage) {
            bool flag = idleCounter.IsPresent();
            bool flag2 = battleUser.Entity.HasComponent<PauseComponent>();
            int num = 0;

            if (flag) {
                float num2 = IdleKickUtils.CalculateTimeLeft(idleCounter.Get().idleCounter,
                    idleCounter.Get().idleBeginTime,
                    idleCounter.Get().idleKickConfig);
        ...
        [OnEventFire]
        public void UpdateGUI(UpdateEvent e, BattleUserCounterWithWarningNode idleCounter,
            [JoinBy(typeof(BattleGroupComponent))] ActiveRoundNode activeRound,
            [JoinAll] VisibleAutokickServiceMessageNode serviceMessage) {
            float num = IdleKickUtils.CalculateTimeLeft(idleCounter.idleCounter,
                idleCounter.idleBeginTime,
                idleCounter.idleKickConfig);
