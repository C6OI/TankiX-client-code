    ...
    public class AbstractNodeDescription : NodeDescription, IComparable<NodeDescription> {
        ...
        protected AbstractNodeDescription(ICollection<Type> components, ICollection<Type> notComponents, ICollection<Type> additionalComponents = null) {
            ...
            if (additionalComponents != null && additionalComponents.Count > 0) {
                foreach (Type item in additionalComponents.Where(c => !BaseComponents.Contains(c))) {
                    components.Add(item);
                }
            }
        ...
        public bool isAdditionalComponents => BaseComponents.Count != Components.Count;

        public ICollection<Type> BaseComponents { get; }
        ...
        string getKey() => string.Join(":", (from c in Components
                                             select c.FullName
                                             into n
                                             orderby n
        ...
        public override string ToString() => "AbstractNodeDescription components: " + EcsToStringUtil.ToString(Components) + " notComponents: " + EcsToStringUtil.ToString(NotComponents);
    }
}