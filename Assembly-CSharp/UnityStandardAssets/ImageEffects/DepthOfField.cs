...
namespace UnityStandardAssets.ImageEffects {
    ...
    public class DepthOfField : PostEffectsBase {
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            if (visualizeFocus) {
                ...
                if (highResolution) {
                    ...
                    dx11bokehMaterial.SetVector("_Screen", new Vector3(1f / (1f * source.width), 1f / (1f * source.height), internalBlurWidth));
                    ...
                    dx11bokehMaterial.SetBuffer("pointBuffer", cbPoints);
                    dx11bokehMaterial.SetTexture("_MainTex", dx11BokehTexture);
                    dx11bokehMaterial.SetVector("_Screen", new Vector3(1f / (1f * renderTexture2.width), 1f / (1f * renderTexture2.height), internalBlurWidth));
                    dx11bokehMaterial.SetPass(1);
                    Graphics.DrawProceduralIndirectNow(MeshTopology.Points, cbDrawArgs);
                    dofHdrMaterial.SetTexture("_LowRez", renderTexture2);
                    dofHdrMaterial.SetTexture("_FgOverlap", renderTexture5);
                    dofHdrMaterial.SetVector("_Offsets", 1f * source.width / (1f * renderTexture2.width) * internalBlurWidth * Vector4.one);
                ...
                if (highResolution) {
                    ...
                    dofHdrMaterial.SetVector("_Offsets", Vector4.one * (1f * source.width / (1f * renderTexture2.width)) * internalBlurWidth);
        ...
        float FocalDistance01(float worldDist) =>
            GetComponent<Camera>()
                .WorldToViewportPoint((worldDist - GetComponent<Camera>().nearClipPlane) * GetComponent<Camera>().transform.forward + GetComponent<Camera>().transform.position).z /
