    ...
    public class NoiseAndScratches : MonoBehaviour {
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            material.SetVector("_GrainOffsetScale",
                new Vector4(Random.value, Random.value, Screen.width / (float)grainTexture.width * num, Screen.height / (float)grainTexture.height * num));

            material.SetVector("_ScratchOffsetScale",
                new Vector4(scratchX + Random.value * scratchJitter, scratchY + Random.value * scratchJitter, Screen.width / (float)scratchTexture.width,
