...
namespace UnityStandardAssets.Water {
    ...
    public class Water : MonoBehaviour {
        ...
        readonly Dictionary<Camera, Camera> m_ReflectionCameras = new();

        RenderTexture m_ReflectionTexture;

        readonly Dictionary<Camera, Camera> m_RefractionCameras = new();
        ...
        void Update() {
            if ((bool)GetComponent<Renderer>()) {
                ...
                if ((bool)sharedMaterial) {
                    ...
                    Vector4 value = new(@float, @float, @float * 0.4f, @float * 0.45f);
                    double num = Time.timeSinceLevelLoad / 20.0;

                    Vector4 value2 = new((float)Math.IEEERemainder(vector.x * value.x * num, 1.0), (float)Math.IEEERemainder(vector.y * value.y * num, 1.0),
                        (float)Math.IEEERemainder(vector.z * value.z * num, 1.0), (float)Math.IEEERemainder(vector.w * value.w * num, 1.0));
        ...
        void OnDisable() {
            ...
            foreach (KeyValuePair<Camera, Camera> reflectionCamera in m_ReflectionCameras) {
                DestroyImmediate(reflectionCamera.Value.gameObject);
            }

            m_ReflectionCameras.Clear();

            foreach (KeyValuePair<Camera, Camera> refractionCamera in m_RefractionCameras) {
                DestroyImmediate(refractionCamera.Value.gameObject);
            }
        ...
        public void OnWillRenderObject() {
            ...
            if ((bool)current && !s_InsideWater) {
                ...
                if (waterMode >= WaterMode.Reflective) {
                    ...
                    Vector4 plane = new(up.x, up.y, up.z, w);
        ...
        void CreateWaterObjects(Camera currentCamera, out Camera reflectionCamera, out Camera refractionCamera) {
            ...
            if (waterMode >= WaterMode.Reflective) {
                ...
                if (!reflectionCamera) {
                    GameObject gameObject = new("Water Refl Camera id" + GetInstanceID() + " for " + currentCamera.GetInstanceID(), typeof(Camera), typeof(Skybox));
            ...
            if (!refractionCamera) {
                GameObject gameObject2 = new("Water Refr Camera id" + GetInstanceID() + " for " + currentCamera.GetInstanceID(), typeof(Camera), typeof(Skybox));
