...
namespace WebSocketSharp {
    internal abstract class HttpBase {
        const int _headersMaxLength = 8192;
        ...
        static byte[] readEntityBody(Stream stream, string length) {
            long result;
        ...
        static string[] readHeaders(Stream stream, int maxLength) {
            List<byte> buff = new();
            int cnt = 0;
        ...
        protected static T Read<T>(Stream stream, Func<string[], T> parser, int millisecondsTimeout) where T : HttpBase {
            ...
            string text2 = timeout ? "A timeout has occurred while reading an HTTP request/response." :
                           ex == null ? null : "An exception has occurred while reading an HTTP request/response.";
