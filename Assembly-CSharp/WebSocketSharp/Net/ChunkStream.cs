...
namespace WebSocketSharp.Net {
    internal class ChunkStream {
        ...
        static string removeChunkExtension(string value) {
            int num = value.IndexOf(';');
            return num <= -1 ? value : value.Substring(0, num);
        }

        InputChunkState seekCrLf(byte[] buffer, ref int offset, int length) {
            if (!_sawCr) {
                if (buffer[offset++] != 13) {
        ...
        InputChunkState setChunkSize(byte[] buffer, ref int offset, int length) {
            byte b = 0;
        ...
        InputChunkState setTrailer(byte[] buffer, ref int offset, int length) {
            ...
            _saved.Length -= 2;
            StringReader stringReader = new(_saved.ToString());
            string text;
        ...
        void write(byte[] buffer, ref int offset, int length) {
        ...
        InputChunkState writeData(byte[] buffer, ref int offset, int length) {
