...
namespace WebSocketSharp.Net {
    ...
    public sealed class Cookie {
        static readonly char[] _reservedCharsForName;

        static readonly char[] _reservedCharsForValue;
        string _comment;
        ...
        public Cookie(string name, string value, string path)
            : this(name, value) => Path = path;

        public Cookie(string name, string value, string path, string domain)
            : this(name, value, path) => Domain = domain;
        ...
        public bool Expired {
            ...
            set => _expires = !value ? DateTime.MinValue : DateTime.Now;
        }
        ...
        static int hash(int i, int j, int k, int l, int m) => i ^ (j << 13 | j >> 19) ^ (k << 26 | k >> 6) ^ (l << 7 | l >> 25) ^ (m << 20 | m >> 12);
        ...
        internal string ToResponseString() => _name.Length <= 0 ? string.Empty : _version != 0 ? toResponseStringVersion1() : toResponseStringVersion0();

        public override bool Equals(object comparand) {
            Cookie cookie = comparand as Cookie;

            return cookie != null && _name.Equals(cookie.Name, StringComparison.InvariantCultureIgnoreCase) && _value.Equals(cookie.Value, StringComparison.InvariantCulture) &&
                   _path.Equals(cookie.Path, StringComparison.InvariantCulture) && _domain.Equals(cookie.Domain, StringComparison.InvariantCultureIgnoreCase) &&
                   _version == cookie.Version;
        }

        public override int GetHashCode() => hash(StringComparer.InvariantCultureIgnoreCase.GetHashCode(_name), _value.GetHashCode(), _path.GetHashCode(),
            StringComparer.InvariantCultureIgnoreCase.GetHashCode(_domain), _version);
