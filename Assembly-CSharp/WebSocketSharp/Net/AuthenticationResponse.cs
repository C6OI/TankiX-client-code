...
namespace WebSocketSharp.Net {
    internal class AuthenticationResponse : AuthenticationBase {
        ...
        AuthenticationResponse(AuthenticationSchemes scheme, NameValueCollection parameters)
        ...
        static string createA1(string username, string password, string realm) => string.Format("{0}:{1}:{2}", username, realm, password);

        static string createA1(string username, string password, string realm, string nonce, string cnonce) =>
            string.Format("{0}:{1}:{2}", hash(createA1(username, password, realm)), nonce, cnonce);

        static string createA2(string method, string uri) => string.Format("{0}:{1}", method, uri);

        static string createA2(string method, string uri, string entity) => string.Format("{0}:{1}:{2}", method, uri, hash(entity));
        ...
        void initAsDigest() {
            ...
            if (text != null) {
                if (text.Split(',').Contains(qop => qop.Trim().ToLower() == "auth")) {
                    Parameters["qop"] = "auth";
        ...
        internal static AuthenticationResponse Parse(string value) {
            try {
                ...
                return text == "basic" ? new AuthenticationResponse(AuthenticationSchemes.Basic, ParseBasicCredentials(array[1])) :
                       !(text == "digest") ? null : new AuthenticationResponse(AuthenticationSchemes.Digest, ParseParameters(array[1]));
            } catch { }
        ...
        internal override string ToDigestString() {
            ...
            stringBuilder.AppendFormat("Digest username=\"{0}\", realm=\"{1}\", nonce=\"{2}\", uri=\"{3}\", response=\"{4}\"", Parameters["username"], Parameters["realm"],
                Parameters["nonce"], Parameters["uri"], Parameters["response"]);
