...
namespace SharpCompress.Compressor.LZMA {
    internal class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties {
        const uint kIfinityPrice = 268435455u;

        const int kDefaultDictionaryLogSize = 22;

        const uint kNumFastBytesDefault = 32u;

        const uint kNumLenSpecSymbols = 16u;

        const uint kNumOpts = 4096u;

        const int kPropSize = 5;

        static readonly byte[] g_FastPos;
        ...
        readonly uint[] _alignPrices = new uint[16];

        uint _dictionarySize = 4194304u;

        uint _dictionarySizePrev = uint.MaxValue;

        readonly uint[] _distancesPrices = new uint[512];

        uint _distTableSize = 44u;

        bool _finished;

        Stream _inStream;

        readonly BitEncoder[] _isMatch = new BitEncoder[192];

        readonly BitEncoder[] _isRep = new BitEncoder[12];

        readonly BitEncoder[] _isRep0Long = new BitEncoder[192];

        readonly BitEncoder[] _isRepG0 = new BitEncoder[12];

        readonly BitEncoder[] _isRepG1 = new BitEncoder[12];

        readonly BitEncoder[] _isRepG2 = new BitEncoder[12];
        ...
        readonly uint[] _matchDistances = new uint[548];
        ...
        readonly Optimal[] _optimum = new Optimal[4096];

        uint _optimumCurrentIndex;

        uint _optimumEndIndex;

        BitTreeEncoder _posAlignEncoder = new(4);

        readonly BitEncoder[] _posEncoders = new BitEncoder[114];

        readonly BitTreeEncoder[] _posSlotEncoder = new BitTreeEncoder[4];

        readonly uint[] _posSlotPrices = new uint[256];

        int _posStateBits = 2;

        uint _posStateMask = 3u;
        ...
        readonly uint[] _repDistances = new uint[4];
        ...
        readonly byte[] properties = new byte[5];

        readonly uint[] repLens = new uint[4];

        readonly uint[] reps = new uint[4];

        readonly uint[] tempPrices = new uint[128];
        ...
        uint GetRepLen1Price(Base.State state, uint posState) => _isRepG0[state.Index].GetPrice0() + _isRep0Long[(state.Index << 4) + posState].GetPrice0();
        ...
        uint GetOptimum(uint position, out uint backRes) {
            ...
            _optimum[1].Price = _isMatch[(_state.Index << 4) + num5].GetPrice0() +
                                _literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!_state.IsCharState(), indexByte2, indexByte);
            ...
            while (true) {
                ...
                uint num20 = price2 + _isMatch[(state.Index << 4) + num5].GetPrice0() +
                             _literalEncoder.GetSubCoder(position, _matchFinder.GetIndexByte(-2)).GetPrice(!state.IsCharState(), indexByte2, indexByte);
                ...
                for (uint num27 = 0u; num27 < 4; num27++) {
                    ...
                    if (matchLen2 >= 2) {
                        ...
                        uint num32 = num6 + GetRepPrice(num27, num28, state, num5) + _isMatch[(state3.Index << 4) + num31].GetPrice0() + _literalEncoder
                                         .GetSubCoder(position + num28, _matchFinder.GetIndexByte((int)(num28 - 1 - 1))).GetPrice(true,
                                             _matchFinder.GetIndexByte((int)(num28 - 1 - (reps[num27] + 1))), _matchFinder.GetIndexByte((int)(num28 - 1)));
                        ...
                        if (num38 < val) {
                            ...
                            if (matchLen3 >= 2) {
                                ...
                                uint num42 = num40 + _isMatch[(state4.Index << 4) + num41].GetPrice0() + _literalEncoder
                                                 .GetSubCoder(position + num38, _matchFinder.GetIndexByte((int)(num38 - 1 - 1))).GetPrice(true,
                                                     _matchFinder.GetIndexByte((int)(num38 - (num39 + 1) - 1)), _matchFinder.GetIndexByte((int)(num38 - 1)));
        ...
        bool ChangePair(uint smallDist, uint bigDist) => smallDist < 33554432 && bigDist >= smallDist << 7;
        ...
        class LiteralEncoder {
            ...
            public Encoder2 GetSubCoder(uint pos, byte prevByte) => m_Coders[(int)((pos & m_PosMask) << m_NumPrevBits) + (prevByte >> 8 - m_NumPrevBits)];
        ...
        class LenEncoder {
            ...
            BitTreeEncoder _highCoder = new(8);

            readonly BitTreeEncoder[] _lowCoder = new BitTreeEncoder[16];

            readonly BitTreeEncoder[] _midCoder = new BitTreeEncoder[16];
        ...
        class LenPriceTableEncoder : LenEncoder {
            readonly uint[] _counters = new uint[16];
            readonly uint[] _prices = new uint[4352];
            ...
            public uint GetPrice(uint symbol, uint posState) => _prices[posState * 272 + symbol];
