...
namespace SharpCompress {
    internal static class Utility {
        ...
        public static void writeIntBigEndian(byte[] array, int pos, int value) {
            array[pos] = (byte)((uint)URShift(value, 24) & 0xFFu);
            array[pos + 1] = (byte)((uint)URShift(value, 16) & 0xFFu);
            array[pos + 2] = (byte)((uint)URShift(value, 8) & 0xFFu);
            array[pos + 3] = (byte)((uint)value & 0xFFu);
        }
        ...
        public static uint DateTimeToDosTime(this DateTime? dateTime) {
            ...
            return (uint)(dateTime.Value.Second / 2 | dateTime.Value.Minute << 5 | dateTime.Value.Hour << 11 | dateTime.Value.Day << 16 | dateTime.Value.Month << 21 |
                          dateTime.Value.Year - 1980 << 25);
        }

        public static DateTime DosDateToDateTime(uint iTime) => DosDateToDateTime((ushort)(iTime / 65536u), (ushort)(iTime % 65536u));
        ...
        internal static void Extract<TEntry, TVolume>(this TEntry entry, AbstractArchive<TEntry, TVolume> archive, Stream streamToWriteTo)
