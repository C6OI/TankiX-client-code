...
namespace Edelweiss.DecalSystem {
    public class DecalsMeshMinimizer {
        ...
        void ComputePotentialObsoleteVertices(DecalsMesh a_DecalsMesh) {
            ...
            for (int k = 0; k < list.Count; k++) {
                ...
                for (int l = k + 1; l < m_RedundantExternalEdges.Count; l++) {
                    ...
                    } else if ((a_Edge4.vertex2Index == a_Edge5.vertex1Index || a_Edge4.vertex2Index == a_Edge5.vertex2Index) &&
                               !m_ObsoleteExternalVertexIndices.Contains(a_Edge4.vertex2Index) && AreEdgesParallelOrIsAtLeastOneAPoint(a_DecalsMesh, a_Edge4, a_Edge5)) {
        ...
        static bool AreEdgesParallelOrIsAtLeastOneAPoint(DecalsMesh a_DecalsMesh, OptimizeEdge a_Edge1, OptimizeEdge a_Edge2) {
            bool result = false;
            ...
            if (Vector3Extension.Approximately(vector5, Vector3.zero, s_CurrentMaximumAbsoluteError, s_CurrentMaximumRelativeError) ||
                Vector3Extension.Approximately(vector6, Vector3.zero, s_CurrentMaximumAbsoluteError, s_CurrentMaximumRelativeError)) {
                result = true;
            } else {
