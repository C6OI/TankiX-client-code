...
namespace Tanks.Battle.ClientCore.API {
    public class TargetingData {
        ...
        public bool HasAnyHit() {
            return Directions.Any(direction => direction.HasAnyHit());
        }

        public bool HasTargetHit() {
            return Directions.Any(direction => direction.HasTargetHit());
        }

        public bool HasBaseStaticHit() => Directions.First().HasStaticHit();

        public bool HasStaticHit() {
            return Directions.Any(direction => direction.HasStaticHit());
        }
    }
}