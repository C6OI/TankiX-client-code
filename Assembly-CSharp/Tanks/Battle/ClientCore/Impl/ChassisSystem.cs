...
namespace Tanks.Battle.ClientCore.Impl {
    public class ChassisSystem : ECSSystem {
        const float SELF_TANK_UPDATE_PERIOD = 0f;

        const float REMOTE_TANK_UPDATE_PERIOD = 0.05f;

        const float REMOTE_INVISBILE_TANK_UPDATE_PERIOD = 0.1f;

        const float REMOTE_RANDOM_TANK_UPDATE_PERIOD = 0.05f;
        ...
        [OnEventFire]
        public void FixedUpdate(FixedUpdateEvent evt, ChassisNode chassisNode, [JoinSelf] Optional<SingleNode<TankSyncComponent>> tankSync,
            [JoinByTank] Optional<SingleNode<TankJumpComponent>> tankJump, [JoinAll] SingleNode<GameTankSettingsComponent> gameTankSettings,
            ...
            if ((bool)rigidbody) {
                ...
                if (x * x + z * z > num * num) {
                    ...
                    b = Mathf.Lerp(1f, b, t);
                    Vector3 velocity = new(rigidbody.velocity.x * b, rigidbody.velocity.y, rigidbody.velocity.z * b);
        ...
        void AdjustSuspensionSpringCoeff(ChassisConfigComponent chassisConfig, ChassisComponent chassis, Rigidbody rigidbody) {
        ...
        bool UpdateSuspensionContacts(TrackComponent trackComponent, float dt, float updatePeriod) {
        ...
        void CalculateNetSurfaceVelocities(ChassisNode node, out Vector3 surfaceVelocity, out Vector3 angularSurfaceVelocity) {
            ChassisConfigComponent chassisConfig = node.chassisConfig;
            TrackComponent track = node.track;
        ...
        void AddSurfaceVelocitiesFromRay(ChassisNode node, SuspensionRay ray, Vector3 contactsMidpoint, ref Vector3 surfaceVelocity, ref Vector3 angularSurfaceVelocity) {
            if (ray.hasCollision) {
        ...
        Vector3 CalculateRigidBodyVelocity(Rigidbody rigidbody, Vector3 surfaceVelocity, float sideSpeedDelta, out float forwardRelativeSpeed) {
        ...
        Vector3 CalculateRelativeAngularVelocity(ChassisNode node, float dt, float slopeCoeff, Vector3 surfaceAngularVelocity, float forwardRelativeSpeed) {
            ChassisConfigComponent chassisConfig = node.chassisConfig;
            TrackComponent track = node.track;
            Rigidbody rigidbody = node.rigidbody.Rigidbody;
        ...
        float CalculateForcePerRay(ChassisNode node, float dt, float forwardRelativeSpeed) {
            ...
            return num9 <= 0 ? num11 : num11 / num9;
        }
        ...
        float RecalculateRelativeTurnSpeed(ChassisNode node, float dt, float maxTurnSpeed, float relativeTurnSpeed, float slopeCoeff) {
            ChassisComponent chassis = node.chassis;
            ChassisConfigComponent chassisConfig = node.chassisConfig;
