...
namespace Tanks.Battle.ClientCore.Impl {
    public class ShaftAimingVerticalTargetingControllerSystem : ECSSystem {
        [OnEventFire]
        public void UpdateVerticalTargetingAngle(WeaponRotationUpdateVerticalEvent e, AimingVerticalTargetingControllerNode weapon,
            ...
            if (mouseControlEnable) {
                ...
                weapon.weaponRotationControl.MouseShaftAimCumulativeVerticalAngle = Mathf.Clamp(weapon.weaponRotationControl.MouseShaftAimCumulativeVerticalAngle,
                    num - weapon.shaftAimingWorkingState.VerticalAngle, vAngleUp - weapon.shaftAimingWorkingState.VerticalAngle);
            } else {
            ...
            if (weapon.shaftAimingWorkingState.VerticalElevationDir != num2) {
                ...
                if (mouseControlEnable) {
                    if (!weapon.weaponRotationControl.BlockRotate) {
                        num3 = num2 <= 0 ? Mathf.Clamp(num3, weapon.weaponRotationControl.MouseShaftAimCumulativeVerticalAngle, 0f)
                                   : Mathf.Clamp(num3, 0f, weapon.weaponRotationControl.MouseShaftAimCumulativeVerticalAngle);
            ...
            weapon.shaftAimingWorkingState.WorkingDirection = Quaternion.AngleAxis(weapon.shaftAimingWorkingState.VerticalAngle, muzzleLogicAccessor.GetLeftDirectionWorld()) *
                                                              muzzleLogicAccessor.GetFireDirectionWorld();

            weapon.shaftAimingWorkingState.IsActive =
                CheckManualTargetingActivity(weapon.weaponRotationControl, weapon.verticalSectorsTargeting, weapon.shaftAimingWorkingState, isInputActive);
        }

        bool CheckManualTargetingActivity(WeaponRotationControlComponent rotation, VerticalSectorsTargetingComponent verticalSectors, ShaftAimingWorkingStateComponent working,
            bool isInputActive) {
