...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class SaveLoadCameraSystem : ECSSystem {
        ...
        public void Save(SaveCameraEvent e, FollowCameraNode camera, [JoinAll] SingleNode<FollowedBattleUserComponent> followedUser, [JoinByUser] UserNode user) {
            CameraSaveData value =
                CameraSaveData.CreateFollowData(user.userUid.Uid, camera.bezierPosition.BezierPosition.GetBaseRatio(), camera.bezierPosition.BezierPosition.GetRatioOffset());
        ...
        [OnEventFire]
        public void LoadCamera(LoadCameraEvent e, SpectatorCameraNode camera, [JoinAll] Optional<SingleNode<FollowedBattleUserComponent>> followedUser,
        ...
        [OnEventFire]
        public void RemoveCurrentCameraController(CameraLoadedSaveValidateEvent e, UserNode user, [JoinByUser] SingleNode<UserInBattleAsTankComponent> userAsTank,
            [JoinByUser] SingleNode<BattleUserComponent> battleUser, [JoinAll] Optional<SingleNode<FollowedBattleUserComponent>> followedUser, [JoinAll] SpectatorCameraNode camera) {
            if (followedUser.IsPresent()) {
                followedUser.Get().Entity.RemoveComponent<FollowedBattleUserComponent>();
            }
