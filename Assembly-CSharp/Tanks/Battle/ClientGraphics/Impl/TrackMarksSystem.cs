...
namespace Tanks.Battle.ClientGraphics.Impl {
    internal class TrackMarksSystem : ECSSystem {
        ...
        public void OnUpdate(UpdateEvent evt, TrackMarkUpdateNode node, [JoinAll] CameraNode cameraNode) {
            ...
            if (num <= 0 || ++node.trackMarks.tick % num == 0) {
                ...
                if (NeedUpdateMarks(trackMarksBuilder, trackMarks)) {
                    UpdateSingleTrack(trackMarksBuilder, chassisAnimation, trackMarksRender, trackMarks, 0, trackMarksBuilder.leftWheels, trackMarksBuilder.prevLeftWheelsData,
                        trackMarksBuilder.currentLeftWheelsData, trackMarksBuilder.tempLeftWheelsData);

                    UpdateSingleTrack(trackMarksBuilder, chassisAnimation, trackMarksRender, trackMarks, 1, trackMarksBuilder.rightWheels, trackMarksBuilder.prevRightWheelsData,
                        trackMarksBuilder.currentRightWheelsData, trackMarksBuilder.tempRightWheelsData);
                }
        ...
        void InitTrackMarksRender(TrackMarksRenderComponent render, TrackMarksComponent trackMarks) {
        ...
        void AddSectorToRender(ref TrackRenderData data, Vector3 startPosition, Vector3 startForward, Vector3 endPosition, Vector3 endForward, Vector3 normal, float width,
            float textureWidth, float rotationCos, bool contiguous) {
        ...
        void ComputeVerticesPosition(ref Vector3 position, ref Vector3 forward, ref Vector3 normal, float width, out Vector3 v0, out Vector3 v1) {
            float num = width / 2f;
        ...
        void ComputeSectorVertices(TrackSector sector, int count, int indexByOrder, int indexInBufer, Vector3 prev0, Vector3 prev1, out float startTextureWidth, out float endTextureWidth,
        ...
        void RotateTextureCoords(ref Vector2 uv1, ref Vector2 uv2, ref Vector2 uv3, ref Vector2 uv4, out Vector2 r1, out Vector2 r2, out Vector2 r3, out Vector2 r4, float c, float s) {
        ...
        void CopySectorToMesh(TrackRenderData data, Vector3 v0, Vector3 v1, Vector3 v2, Vector3 v3, Vector3 normal, int indexByOrder, Vector3[] positions, Vector2[] uvs,
            Vector3[] normals, Color[] colors, int[] triangles, int firstVertex, int firstIndex, float halfAlpha, float startTextureWidth, float endTextureWidth, float rotationCos) {
        ...
        void CopyToMesh(TrackRenderData data, Vector3[] positions, Vector2[] uvs, Vector3[] normals, Color[] colors, int[] triangles, int firstVertex, int firstIndex, out int nextIndex,
            out int nextVertex) {
            ...
            float halfAlpha = 0.5f / (data.maxSectorCountPerTrack - data.firstSectorToHide);
            ...
            for (int num2 = data.sectorCount; num2 > 0; num2--) {
                ...
                CopySectorToMesh(data, v, v2, v3, v4, sector.normal, num2, positions, uvs, normals, colors, triangles, firstVertex, firstIndex, halfAlpha, startTextureWidth,
                    endTextureWidth, sector.rotationCos);
        ...
        void UpdateMesh(TrackMarksRenderComponent data) {
            ...
            for (int i = 0; i < 2; i++) {
                CopyToMesh(data.trackRenderDatas[i], data.meshPositions, data.meshUVs, data.meshNormals, data.meshColors, data.meshTris, nextVertex, nextIndex, out nextIndex,
                    out nextVertex);
            }
        ...
        void MakeTracks(TrackMarksRenderComponent trackMarksRender, int track, Vector3 startPosition, Vector3 startForward, Vector3 endPosition, Vector3 endForward, Vector3 normal,
            float width, float textureWidth, float rotationCos, bool contiguous) {
        ...
        static void AllocateBuilder(TrackMarksBuilderComponent builder, TrackMarksComponent component, TrackMarksRenderComponent renderer) {
        ...
        void FindWheels(TrackMarksBuilderComponent builder, ChassisAnimationComponent chassis, TrackMarksComponent trackMarks) {
        ...
        bool CheckDirectionChange(TrackMarksBuilderComponent builder, TrackMarksComponent trackMarks, int i, ref RaycastHit hitData) {
        ...
        bool CheckNormalChange(TrackMarksBuilderComponent builder, TrackMarksComponent trackMarks, int i, ref RaycastHit hitData) {
        ...
        void UpdateTrack(TrackMarksBuilderComponent builder, TrackMarksRenderComponent render, TrackMarksComponent trackMarks, int track, bool hit, ref RaycastHit hitData, float width,
            float shiftToBack) {
            if (hit) {
                if (!builder.prevHits[track]) {
                    ...
                    builder.nextPositions[track] =
                        hitData.point - trackMarks.transform.forward * shiftToBack * trackMarks.markWidth / 2f + normalized * shiftToBack * trackMarks.markWidth;

                    builder.nextNormals[track] = hitData.normal;

                    if (CheckEnoughMove(builder, track, ref hitData) && CheckDirectionChange(builder, trackMarks, track, ref hitData) &&
                        ...
                        MakeTracks(render, track, builder.positions[track], builder.directions[track], builder.nextPositions[track], normalized, hitData.normal, width,
                            width / trackMarks.markWidth, num, builder.contiguous[track]);
        ...
        bool CheckExtraContacts(TrackMarksBuilderComponent builder, TrackMarksComponent trackMarks, WheelData[] wheels, float hitDistance) {
            ...
            return Physics.Raycast(wheels[0].position, -trackMarks.transform.up, maxDistance, TRACK_LAYER_MASK) ||
                   !Physics.Raycast(wheels[wheels.Length - 1].position, -trackMarks.transform.up, maxDistance, TRACK_LAYER_MASK);
        }

        bool IsHit(TrackMarksBuilderComponent builder, TrackMarksComponent trackMarks, WheelData[] wheels, float hitDistance, out RaycastHit hit) {
        ...
        Vector3 ComputeTrackDiagonal(TrackMarksBuilderComponent builder, TrackMarksComponent trackMarks, WheelData[] data, ref Vector3 left) {
        ...
        void UpdateWheels(TrackMarksBuilderComponent builder, ChassisAnimationComponent chassis, TrackMarksRenderComponent render, TrackMarksComponent trackMarks, int track,
            WheelData[] currentWheelsPositions, WheelData[] prevWheelsPosition, WheelData[] temp) {
        ...
        void UpdateSingleTrack(TrackMarksBuilderComponent builder, ChassisAnimationComponent chassis, TrackMarksRenderComponent render, TrackMarksComponent trackMarks, int track,
            Transform[] wheels, WheelData[] prevWheelsData, WheelData[] currentWheelsData, WheelData[] tempWheelsData) {
        ...
        void ResetBuilder(TrackMarksBuilderComponent builder) {
            for (int i = 0; i < 2; i++) {
                builder.prevHits[i] = false;
                builder.contiguous[i] = false;
