...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class TrackController {
        ...
        void AnimateTrackPointsWithAdditionalRays(Transform transform) {
            for (int i = 0; i < segments.Length; i++) {
                ...
                if (Physics.Raycast(vector, vector3, out fromHit, magnitude, LayerMasks.VISIBLE_FOR_CHASSIS_ANIMATION)) {
                    ...
                    Vector3 vector4 = !Physics.Raycast(vector2, vector3, out toHit, magnitude, LayerMasks.VISIBLE_FOR_CHASSIS_ANIMATION) ? fromHit.point
                                          : Vector3.Lerp(fromHit.point, toHit.point, 0.5f);
        ...
        static Vector3 GetPointWithVerticalOffset(Vector3 initialPoint, float verticalOffset) => new(initialPoint.x, initialPoint.y + verticalOffset, initialPoint.z);

        static Vector3 GetPointWithVerticalPosition(Vector3 point, float verticalPosition) => new(point.x, verticalPosition, point.z);
    }
}