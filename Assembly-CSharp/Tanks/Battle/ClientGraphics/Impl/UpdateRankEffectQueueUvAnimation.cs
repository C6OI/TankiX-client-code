...
namespace Tanks.Battle.ClientGraphics.Impl {
    public class UpdateRankEffectQueueUvAnimation : MonoBehaviour {
        ...
        void InitDefaultTex(int rows, int colums) {
            count = rows * colums;
            index += colums - 1;
            Vector2 value = new(1f / colums, 1f / rows);
        ...
        IEnumerator UpdateTiling() {
            while (isVisible && allCount != count) {
                ...
                Vector2 offset = isFadeHandle ? new Vector2(index / (float)ColumnsLoop - index / ColumnsLoop, 1f - index / ColumnsLoop / (float)RowsLoop)
                                     : new Vector2(index / (float)ColumnsFadeIn - index / ColumnsFadeIn, 1f - index / ColumnsFadeIn / (float)RowsFadeIn);
