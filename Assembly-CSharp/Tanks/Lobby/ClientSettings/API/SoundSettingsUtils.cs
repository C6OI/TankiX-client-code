...
namespace Tanks.Lobby.ClientSettings.API {
    public class SoundSettingsUtils {
        ...
        static readonly Dictionary<SoundType, SoundSettingsKeys> keys = new() {
            {
                SoundType.SFX,
        ...
        public static float GetSavedVolume(SoundType type) => PlayerPrefs.GetFloat(keys[type].volumeKey, keys[type].defaultVolumeValue);

        public static void SaveVolume(SoundType type, float volume) {
            PlayerPrefs.SetFloat(keys[type].volumeKey, volume);
        }

        public static bool GetSavedMuteFlag(SoundType type) => PlayerPrefs.GetInt(keys[type].muteFlagKey, 0) == 1;
