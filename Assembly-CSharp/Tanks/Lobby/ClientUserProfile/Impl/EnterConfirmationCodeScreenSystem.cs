...
namespace Tanks.Lobby.ClientUserProfile.Impl {
    public class EnterConfirmationCodeScreenSystem : ECSSystem {
        [OnEventFire]
        public void Init(NodeAddedEvent e, SingleNode<EnterConfirmationCodeScreenComponent> screen, SingleNode<RestorePasswordCodeSentComponent> data,
            [JoinAll] SingleNode<EmailConfirmationCodeConfigComponent> emailSendConfig) {
            EnterConfirmationCodeScreenComponent component = screen.component;

            component.ConfirmationHintWithUserEmail =
                component.ConfirmationHint.Replace("%EMAIL%", string.Format("<color=#{1}>{0}</color>", data.component.Email, component.EmailColor.ToHexString()));

            screen.component.ResetSendAgainTimer(emailSendConfig.component.EmailSendThresholdInSeconds);
        }

        [OnEventFire]
        public void SendAgain(ButtonClickEvent e, SingleNode<SendAgainButtonComponent> button, [JoinByScreen] SingleNode<EnterConfirmationCodeScreenComponent> screen,
            [JoinAll] SingleNode<RestorePasswordCodeSentComponent> session, [JoinAll] SingleNode<EmailConfirmationCodeConfigComponent> emailSendConfig) {
            ScheduleEvent<SendAgainRestorePasswordEvent>(session);
            screen.component.ResetSendAgainTimer(emailSendConfig.component.EmailSendThresholdInSeconds);
        }

        [OnEventFire]
        public void SendCode(ButtonClickEvent e, SingleNode<ContinueButtonComponent> button, [JoinByScreen] CodeInputNode code,
            [JoinByScreen] SingleNode<EnterConfirmationCodeScreenComponent> screen, [JoinAll] SingleNode<ClientSessionComponent> session) {
        ...
        [OnEventFire]
        public void InvalidCode(RestorePasswordCodeInvalidEvent e, SingleNode<ClientSessionComponent> session, [JoinAll] SingleNode<EnterConfirmationCodeScreenComponent> screen,
            [JoinByScreen] CodeInputNode code) {
        ...
        [OnEventFire]
        public void ValidCode(RestorePasswordCodeValidEvent e, SingleNode<ClientSessionComponent> session, [JoinAll] SingleNode<EnterConfirmationCodeScreenComponent> screen,
            [JoinByScreen] CodeInputNode code) {
