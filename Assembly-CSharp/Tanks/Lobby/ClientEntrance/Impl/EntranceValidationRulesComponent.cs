...
namespace Tanks.Lobby.ClientEntrance.Impl {
    public class EntranceValidationRulesComponent : Component {
        ...
        public bool IsLoginValid(string login) => !IsLoginTooShort(login) && !IsLoginTooLong(login) && IsLoginSymbolsValid(login) && IsLoginBeginingValid(login) &&
                                                  IsLoginEndingValid(login) && !AreSpecSymbolsTogether(login) && loginRegex.IsMatch(login);
    }
}