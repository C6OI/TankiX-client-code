...
namespace Tanks.Lobby.ClientControls.Impl {
    public class CombatEventLog : MonoBehaviour, UILog {
        ...
        [SerializeField] bool attachToRight = true;
        ...
        readonly Regex userPattern = new("{[0-9][0-9]?[0-9]?:[0-9a-fA-F]*:[^}]*}");
        ...
        void OnScrollLog(float height) {
            scrollHeight += height + spaceBetweenMessages;
        }

        void OnDeleteMessage(CombatEventLogMessage message) {
