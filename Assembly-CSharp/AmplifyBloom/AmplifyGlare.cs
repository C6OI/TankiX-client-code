...
namespace AmplifyBloom {
    ...
    public sealed class AmplifyGlare : IAmplifyItem {
        ...
        [SerializeField] GlareDefData[] m_customGlareDef;

        [SerializeField] int m_customGlareDefIdx;

        [SerializeField] int m_customGlareDefAmount;

        [SerializeField] bool m_applyGlare = true;

        [SerializeField] Color _overallTint = Color.white;

        [SerializeField] Gradient m_cromaticAberrationGrad;

        [SerializeField] int m_glareMaxPassCount = 4;

        [SerializeField] int m_currentWidth;

        [SerializeField] int m_currentHeight;

        [SerializeField] GlareLibType m_currentGlareType;

        [SerializeField] int m_currentGlareIdx;

        [SerializeField] float m_perPassDisplacement = 4f;

        [SerializeField] float m_intensity = 0.17f;

        [SerializeField] float m_overallStreakScale = 1f;

        RenderTexture[] _rtBuffer;

        AmplifyGlareCache m_amplifyGlareCache;

        float m_aTanFoV;

        GlareDefData[] m_glareDefArr;

        bool m_isDirty = true;

        Matrix4x4[] m_offsetsMat;

        StarDefData[] m_starDefArr;

        Matrix4x4[] m_weigthsMat;

        Color m_whiteReference;
        ...
        public AmplifyGlare() {
            ...
            m_starDefArr = new StarDefData[5] {
                new(StarLibType.Cross, "Cross", 2, 4, 1f, 0.85f, 0f, 0.5f, -1f, 90f),
                new(StarLibType.Cross_Filter, "CrossFilter", 2, 4, 1f, 0.95f, 0f, 0.5f, -1f, 90f),
                new(StarLibType.Snow_Cross, "snowCross", 3, 4, 1f, 0.96f, 0.349f, 0.5f, -1f),
                new(StarLibType.Vertical, "Vertical", 1, 4, 1f, 0.96f, 0f, 0f, -1f),
                new(StarLibType.Sunny_Cross, "SunnyCross", 4, 4, 1f, 0.88f, 0f, 0f, 0.95f, 45f)
            };

            m_glareDefArr = new GlareDefData[9] {
                new(StarLibType.Cross, 0f, 0.5f),
        ...
        public void OnRenderFromCache(RenderTexture source, RenderTexture dest, Material material, float glareIntensity, float cameraRotation) {
            ...
            for (int j = 0; j < m_amplifyGlareCache.StarDef.StarlinesCount; j++) {
                for (int k = 0; k < m_amplifyGlareCache.CurrentPassCount; k++) {
                    UpdateMatrixesForPass(material, m_amplifyGlareCache.Starlines[j].Passes[k].Offsets, m_amplifyGlareCache.Starlines[j].Passes[k].Weights, glareIntensity,
        ...
        public void OnRenderImage(Material material, RenderTexture source, RenderTexture dest, float cameraRot) {
            ...
            if (m_isDirty || m_currentWidth != source.width || m_currentHeight != source.height) {
                ...
                for (int i = 0; i < m_glareMaxPassCount; i++) {
                    float t = (i + 1) / (float)m_glareMaxPassCount;

                    for (int j = 0; j < 8; j++) {
                        Color b = _overallTint * Color.Lerp(m_cromaticAberrationGrad.Evaluate(j / 7f), m_whiteReference, t);
                ...
                for (int l = 0; l < starDefData.StarlinesCount; l++) {
                    ...
                    for (int m = 0; m < num3; m++) {
                        for (int n = 0; n < 8; n++) {
                            float num9 = Mathf.Pow(starLineData.Attenuation, num8 * n);
                            m_amplifyGlareCache.Starlines[l].Passes[m].Weights[n] = m_amplifyGlareCache.CromaticAberrationMat[num3 - 1 - m, n] * num9 * (m + 1f) * 0.5f;
                            m_amplifyGlareCache.Starlines[l].Passes[m].Offsets[n].x = vector.x * n;
                            m_amplifyGlareCache.Starlines[l].Passes[m].Offsets[n].y = vector.y * n;
                        ...
                        UpdateMatrixesForPass(material, m_amplifyGlareCache.Starlines[l].Passes[m].Offsets, m_amplifyGlareCache.Starlines[l].Passes[m].Weights, m_intensity,
