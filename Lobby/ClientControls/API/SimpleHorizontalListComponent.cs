...
namespace Lobby.ClientControls.API {
    public class SimpleHorizontalListComponent : MonoBehaviour, IEventSystemHandler, IBeginDragHandler, IDragHandler,
        ...
        public ICollection<Entity> GetItems() => items.Select(item => (Entity)item.Data).ToList();
        ...
        public void SetIndex(Entity entity, int index) => items[entity].transform.SetSiblingIndex(index);
        ...
        float GetMinPosition() =>
            Mathf.Min(0f, scrollRect.rect.width - CalculateItemsWidth(items.Count, calculatedItemWidth));
        ...
        void ToNearestItem() {
            ...
            targetItemIndex = !(dragDirection < 0f) ? PositionToItemIndex(position - num)
                                  : PositionToItemIndex(position - (calculatedItemWidth + spaceBetweenElements + num));
        ...
        void OnItemSelect(ListItem item) {
            ...
            ClientUnityIntegrationUtils.ExecuteInFlow(delegate(Engine engine) {
                engine.ScheduleEvent<ListItemSelectedEvent>((Entity)selectedItem.Data);
            });
        }
