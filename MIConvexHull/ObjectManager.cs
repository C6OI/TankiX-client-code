...
namespace MIConvexHull {
    class ObjectManager {
        readonly SimpleList<DeferredFace> DeferredFaceStack;
        readonly int Dimension;

        readonly SimpleList<IndexBuffer> EmptyBufferStack;

        readonly IndexBuffer FreeFaceIndices;

        readonly ConvexHullInternal Hull;

        FaceConnector ConnectorStack;

        ConvexFaceInternal[] FacePool;

        int FacePoolCapacity;
        ...
        void ReallocateFacePool() {
            ...
            Buffer.BlockCopy(Hull.AffectedFaceFlags, 0, array2, 0, FacePoolCapacity * 1);
        ...
        int CreateFace() {
            ...
            ConvexFaceInternal convexFaceInternal = new(Dimension, facePoolSize, GetVertexBuffer());
            FacePoolSize++;
        ...
        public IndexBuffer GetVertexBuffer() => EmptyBufferStack.Count == 0 ? new IndexBuffer() : EmptyBufferStack.Pop();

        public void DepositDeferredFace(DeferredFace face) => DeferredFaceStack.Push(face);

        public DeferredFace GetDeferredFace() => DeferredFaceStack.Count == 0 ? new DeferredFace() : DeferredFaceStack.Pop();
    }
}