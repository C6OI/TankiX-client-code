        ...
        void Update() {
            if ((bool)GetComponent<Renderer>()) {
                ...
                if ((bool)sharedMaterial) {
                    ...
                    Vector4 vector2 = new(@float, @float, @float * 0.4f, @float * 0.45f);
                    double num = Time.timeSinceLevelLoad / 20.0;

                    Vector4 vector3 = new((float)Math.IEEERemainder(vector.x * vector2.x * num, 1.0),
            ...
            if (waterMode >= WaterMode.Reflective) {
                ...
                if (!reflectionCamera) {
                    GameObject gameObject =
                        new("Water Refl Camera id" + GetInstanceID() + " for " + currentCamera.GetInstanceID(),
                            typeof(Camera),
                            typeof(Skybox));
