...
namespace UnityStandardAssets.ImageEffects {
    ...
    public class SunShafts : PostEffectsBase {
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            vector = !sunTransform ? new Vector3(0.5f, 0.5f, 0f)
            ...
            if (!useDepthTexture) {
                RenderTextureFormat format = !GetComponent<Camera>().allowHDR ? RenderTextureFormat.Default
                                                 : RenderTextureFormat.DefaultHDR;

                RenderTexture renderTexture =
                    RenderTexture.active = RenderTexture.GetTemporary(source.width, source.height, 0, format);
