...
namespace UnityStandardAssets.ImageEffects {
    ...
    public class DepthOfFieldDeprecated : PostEffectsBase {
        ...
        static readonly int SMOOTH_DOWNSAMPLE_PASS = 6;

        static readonly float BOKEH_EXTRA_BLUR = 2f;
        ...
        float focalEndCurve = 2f;

        float focalStartCurve = 2f;
        ...
        float oneOverBaseSize = 0.001953125f;
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            if (simpleTweakMode) {
                focalDistance01 = !objectFocus ? FocalDistance01(focalPoint)
            ...
            dofMaterial.SetVector("_CurveParams",
                new Vector4(!simpleTweakMode ? focalStartCurve : 1f / focalStartCurve,
                    !simpleTweakMode ? focalEndCurve : 1f / focalEndCurve,
                    num2 * 0.5f,
                    focalDistance01));

            dofMaterial.SetVector("_InvRenderTargetSize",
                new Vector4(1f / (1f * source.width), 1f / (1f * source.height), 0f, 0f));
            ...
            if (bokeh && (BokehDestination.Foreground & bokehDestination) != 0) {
                dofMaterial.SetVector("_Threshhold",
                    new Vector4(bokehThresholdContrast, bokehThresholdLuminance, 0.95f, 0f));
            ...
            if (flag) {
                ...
                if (bokeh && (BokehDestination.Foreground & bokehDestination) != 0) {
                    dofMaterial.SetVector("_Threshhold",
                        new Vector4(bokehThresholdContrast * 0.5f, bokehThresholdLuminance, 0f, 0f));
            ...
            if (iterations > DofBlurriness.Low) {
                ...
                if (iterations > DofBlurriness.High) {
                    ...
                    dofBlurMaterial.SetVector("offsets",
                        new Vector4(spread / widthOverHeight * oneOverBaseSize, 0f, 0f, 0f));
        ...
        void BlurHex(RenderTexture from, RenderTexture to, int blurPass, float spread, RenderTexture tmp) {
            ...
            dofBlurMaterial.SetVector("offsets",
                new Vector4(spread / widthOverHeight * oneOverBaseSize, spread * oneOverBaseSize, 0f, 0f));

            Graphics.Blit(to, tmp, dofBlurMaterial, blurPass);

            dofBlurMaterial.SetVector("offsets",
                new Vector4(spread / widthOverHeight * oneOverBaseSize, (0f - spread) * oneOverBaseSize, 0f, 0f));
        ...
        void AddBokeh(RenderTexture bokehInfo, RenderTexture tempTex, RenderTexture finalTarget) {
            ...
            float num2 = 2f / (1f * bokehInfo.width);
