...
namespace UnityStandardAssets.ImageEffects {
    ...
    public class CameraMotionBlur : PostEffectsBase {
        ...
        static readonly float MAX_RADIUS = 10f;
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            RenderTextureFormat format = !SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RGHalf)
                                             ? RenderTextureFormat.ARGBHalf : RenderTextureFormat.RGHalf;

            RenderTexture temporary = RenderTexture.GetTemporary(divRoundUp(source.width, velocityDownsample),
            ...
            if (filterType == MotionBlurFilter.CameraMotion) {
                ...
                float num5 = 1f;
                num5 = Vector3.Angle(transform.up, prevFrameUp) / _camera.fieldOfView * (source.width * 0.75f);
                zero.x = rotationScale * num5;
                num5 = Vector3.Angle(transform.forward, prevFrameForward) / _camera.fieldOfView * (source.width * 0.75f);
                zero.y = rotationScale * num4 * num5;
                num5 = Vector3.Angle(transform.forward, prevFrameForward) / _camera.fieldOfView * (source.width * 0.75f);
                zero.z = rotationScale * (1f - num4) * num5;

                if (magnitude > Mathf.Epsilon && movementScale > Mathf.Epsilon) {
                    zero.w = movementScale * Vector3.Dot(transform.forward, rhs) * (source.width * 0.5f);
                    zero.x += movementScale * Vector3.Dot(transform.up, rhs) * (source.width * 0.5f);
                    zero.y += movementScale * Vector3.Dot(transform.right, rhs) * (source.width * 0.5f);
                }

                if (preview) {
                    motionBlurMaterial.SetVector("_BlurDirectionPacked",
                        new Vector4(previewScale.y, previewScale.x, 0f, previewScale.z) * 0.5f * _camera.fieldOfView);
                } else {
        ...
        void StartFrame() => prevFramePos = Vector3.Slerp(prevFramePos, transform.position, 0.75f);
