...
namespace UnityStandardAssets.ImageEffects {
    ...
    public class DepthOfField : PostEffectsBase {
        ...
        float focalDistance01 = 10f;

        float internalBlurWidth = 1f;

        void OnEnable() => GetComponent<Camera>().depthTextureMode |= DepthTextureMode.Depth;
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            focalDistance01 = !focalTransform ? FocalDistance01(focalLength)
            ...
            if (visualizeFocus) {
                ...
                if (highResolution) {
                    ...
                    dx11bokehMaterial.SetVector("_BokehParams",
                        new Vector4(dx11BokehScale,
                            dx11BokehIntensity,
                            Mathf.Clamp(dx11BokehThreshold, 0.005f, 4f),
                            internalBlurWidth));
                    ...
                    dx11bokehMaterial.SetVector("_Screen",
                        new Vector3(1f / (1f * source.width), 1f / (1f * source.height), internalBlurWidth));
                    ...
                    renderTexture3 = RenderTexture.GetTemporary(renderTexture.width >> 1,
                        renderTexture.height >> 1,
                        0,
                        renderTexture.format);

                    renderTexture4 = RenderTexture.GetTemporary(renderTexture.width >> 1,
                        renderTexture.height >> 1,
                        0,
                        renderTexture.format);
                    ...
                    dx11bokehMaterial.SetVector("_Screen",
                        new Vector3(1f / (1f * renderTexture2.width), 1f / (1f * renderTexture2.height), internalBlurWidth));
                    ...
                    dofHdrMaterial.SetVector("_Offsets",
                        1f * source.width / (1f * renderTexture2.width) * internalBlurWidth * Vector4.one);
                ...
                if (highResolution) {
                    ...
                    dofHdrMaterial.SetVector("_Offsets",
                        Vector4.one * (1f * source.width / (1f * renderTexture2.width)) * internalBlurWidth);
        ...
        float FocalDistance01(float worldDist) =>
            GetComponent<Camera>()
                .WorldToViewportPoint(
                    (worldDist - GetComponent<Camera>().nearClipPlane) * GetComponent<Camera>().transform.forward +
