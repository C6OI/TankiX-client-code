...
namespace MeshBrush {
    public class RuntimeAPI : MonoBehaviour {
        ...
        public Vector4 randomScale {
            get => _randomScale;
            set => _randomScale = new Vector4(Mathf.Clamp(value.x, 0.01f, value.x),
                       Mathf.Clamp(value.y, 0.01f, value.y),
                       Mathf.Clamp(value.z, 0.01f, value.z),
        ...
        public void Paint_SingleMesh(RaycastHit paintHit) {
            ...
            slopeAngle = activeSlopeFilter
                             ? Vector3.Angle(paintHit.normal, !manualRefVecSampling ? Vector3.up : sampledSlopeRefVector)
                             : !inverseSlopeFilter
                                 ? 0f
                                 : 180f;

            if (!inverseSlopeFilter ? slopeAngle < maxSlopeFilterAngle : slopeAngle > maxSlopeFilterAngle) {
                paintedMesh = Instantiate(setOfMeshesToPaint[Random.Range(0, setOfMeshesToPaint.Length)],
                    paintHit.point,
                    Quaternion.LookRotation(paintHit.normal));

                paintedMeshTransform = paintedMesh.transform;

                if (yAxisIsTangent) {
                    paintedMeshTransform.up = Vector3.Lerp(paintedMeshTransform.up,
                        paintedMeshTransform.forward,
                        slopeInfluence * 0.01f);
                } else {
        ...
        public void Paint_MultipleMeshes(RaycastHit paintHit) {
            ...
            for (int num = amount; num > 0; num--) {
                ...
                brushTransform.Translate(
                    Random.Range((0f - Random.insideUnitCircle.x) * scatteringInsetThreshold,
                        Random.insideUnitCircle.x * scatteringInsetThreshold),
                    0f,
                    Random.Range((0f - Random.insideUnitCircle.y) * scatteringInsetThreshold,
                        Random.insideUnitCircle.y * scatteringInsetThreshold),
                    Space.Self);

                if (Physics.Raycast(brushTransform.position, -paintHit.normal, out hit, 2.5f)) {
                    slopeAngle = activeSlopeFilter
                                     ? Vector3.Angle(hit.normal, !manualRefVecSampling ? Vector3.up : sampledSlopeRefVector)
                                     : !inverseSlopeFilter
                                         ? 0f
                                         : 180f;
                    ...
                    if (!inverseSlopeFilter ? slopeAngle < maxSlopeFilterAngle : slopeAngle > maxSlopeFilterAngle) {
                        ...
                        if (yAxisIsTangent) {
                            ...
                            paintedMeshTransform.up = Vector3.Lerp(Vector3.up,
                                paintedMeshTransform.forward,
                                slopeInfluence * 0.01f);
                        }
        ...
        void AddConstantScale(GameObject sMesh) => sMesh.transform.localScale +=
                                                       new Vector3(Mathf.Clamp(additiveScale.x, -0.9f, additiveScale.x),
                                                           Mathf.Clamp(additiveScale.y, -0.9f, additiveScale.y),
