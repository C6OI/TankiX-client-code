...
namespace YamlDotNet.Core {
    public sealed class MergingParser : IParser {
        readonly List<ParsingEvent> _allEvents = new();

        readonly IParser _innerParser;

        int _currentIndex = -1;

        public MergingParser(IParser innerParser) => _innerParser = innerParser;
            ...
            if (_currentIndex < 0) {
                ...
                for (int num = _allEvents.Count - 2; num >= 0; num--) {
                    ...
                    if (sequenceStart != null) {
                        List<IEnumerable<ParsingEvent>> list = new();
                        bool flag = false;
                        ...
                        for (int i = num + 2; i < _allEvents.Count; i++) {
                            ...
                            if (anchorAlias != null) {
                                ...
                                _allEvents.InsertRange(num, list.SelectMany(e => e));
                                flag = true;
                                break;
                            }
                        }
        ...
        IEnumerable<ParsingEvent> GetMappingEvents(string mappingAlias) {
            ParsingEventCloner cloner = new();
            int nesting = 0;
            ...
            return (from e in _allEvents.SkipWhile(delegate(ParsingEvent e) {
                    ...
                    }).Skip(1).TakeWhile(e => (nesting += e.NestingIncrease) >= 0)
                    select cloner.Clone(e)).ToList();
        }

        class ParsingEventCloner : IParsingEventVisitor {
            ParsingEvent clonedEvent;
            ...
            void IParsingEventVisitor.Visit(Scalar e) => clonedEvent = new Scalar(null,
                                                             e.Tag,
                                                             e.Value,
                                                             e.Style,
                                                             e.IsPlainImplicit,
