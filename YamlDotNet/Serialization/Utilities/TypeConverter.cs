...
namespace YamlDotNet.Serialization.Utilities {
    public static class TypeConverter {
        [PermissionSet((SecurityAction)14,
            XML = "<PermissionSet class=\"System.Security.PermissionSet\"\nversion=\"1\"/>\n")]
        public static void RegisterTypeConverter<TConvertible, TConverter>()
            where TConverter : System.ComponentModel.TypeConverter {
            if (!TypeDescriptor.GetAttributes(typeof(TConvertible)).OfType<TypeConverterAttribute>()
        ...
        public static T ChangeType<T>(object value, IFormatProvider provider) => (T)ChangeType(value, typeof(T), provider);

        public static T ChangeType<T>(object value, CultureInfo culture) => (T)ChangeType(value, typeof(T), culture);

        public static object ChangeType(object value, Type destinationType) =>
            ChangeType(value, destinationType, CultureInfo.InvariantCulture);

        public static object ChangeType(object value, Type destinationType, IFormatProvider provider) => ChangeType(value,
            destinationType,
            new CultureInfoAdapter(CultureInfo.CurrentCulture, provider));
        ...
        public static object ChangeType(object value, Type destinationType, CultureInfo culture) {
            ...
            foreach (Type type2 in array) {
                foreach (MethodInfo publicStaticMethod2 in type2.GetPublicStaticMethods()) {
                    if (!publicStaticMethod2.IsSpecialName ||
                        !(publicStaticMethod2.Name == "op_Implicit") && !(publicStaticMethod2.Name == "op_Explicit") ||
                        !destinationType.IsAssignableFrom(publicStaticMethod2.ReturnParameter.ParameterType)) {
                        continue;
                    }
