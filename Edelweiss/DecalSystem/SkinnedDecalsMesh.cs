...
namespace Edelweiss.DecalSystem {
    public class SkinnedDecalsMesh : GenericDecalsMesh<SkinnedDecals, SkinnedDecalProjectorBase, SkinnedDecalsMesh> {
        ...
        public SkinnedDecalsMesh(SkinnedDecals a_Decals) => m_Decals = a_Decals;

        public List<Vector3> OriginalVertices { get; } = new();

        public List<BoneWeight> BoneWeights { get; } = new();
        ...
        internal override void SetRangesForAddedProjector(SkinnedDecalProjectorBase a_Projector) {
            ...
            a_Projector.DecalsMeshLowerBonesIndex = Bones.Count;
            a_Projector.DecalsMeshUpperBonesIndex = Bones.Count - 1;
        }

        public void Add(Mesh a_Mesh, Transform[] a_Bones, SkinQuality a_SkinQuality, Matrix4x4 a_WorldToMeshMatrix,
            Matrix4x4 a_MeshToWorldMatrix) {
            SkinnedDecalProjectorBase activeDecalProjector = ActiveDecalProjector;

            if (activeDecalProjector == null) {
                throw new NullReferenceException(
                    "The active decal projector is not allowed to be null as mesh data should be added!");
            }
            ...
            if (m_Decals.CurrentUVMode == UVMode.Project || m_Decals.CurrentUVMode == UVMode.ProjectWrapped) {
                ...
                if (0 > activeDecalProjector.UV1RectangleIndex ||
                    activeDecalProjector.UV1RectangleIndex >= m_Decals.uvRectangles.Length) {
                    throw new IndexOutOfRangeException(
                        "The uv rectangle index of the active projector is not a valid index within the decals uv rectangles array!");
                }
            }
            ...
            if (m_Decals.CurrentUV2Mode == UV2Mode.Project || m_Decals.CurrentUV2Mode == UV2Mode.ProjectWrapped) {
                ...
                if (0 > activeDecalProjector.UV2RectangleIndex ||
                    activeDecalProjector.UV2RectangleIndex >= m_Decals.uv2Rectangles.Length) {
                    throw new IndexOutOfRangeException(
                        "The uv2 rectangle index of the active projector is not a valid index within the decals uv2 rectangles array!");
                }
            }
        ...
        public void Add(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Vector2[] a_UVs, Vector2[] a_UV2s,
            BoneWeight[] a_BoneWeights, int[] a_Triangles, Transform[] a_Bones, Matrix4x4[] a_BindPoses,
            SkinQuality a_SkinQuality, Matrix4x4 a_WorldToMeshMatrix, Matrix4x4 a_MeshToWorldMatrix) => Add(a_Vertices,
            a_Normals,
            a_Tangents,
            null,
            a_UVs,
        ...
        public void Add(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Color[] a_Colors, Vector2[] a_UVs,
            Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, Transform[] a_Bones, Matrix4x4[] a_BindPoses,
            SkinQuality a_SkinQuality, Matrix4x4 a_WorldToMeshMatrix, Matrix4x4 a_MeshToWorldMatrix) {
            SkinnedDecalProjectorBase activeDecalProjector = ActiveDecalProjector;
            ...
            for (int i = 0; i < a_Bones.Length; i++) {
                Bones.Add(a_Bones[i]);
                BindPoses.Add(a_BindPoses[i] * a_MeshToDecalsMatrix.inverse);
            }

            InternalAdd(a_Vertices,
                a_Normals,
                a_Tangents,
                a_Colors,
                a_UVs,
            ...
            if (!(num < 0f)) {
                for (int j = 0; j < a_Triangles.Length; j += 3) {
                    ...
                    if (!m_RemovedIndices.IsRemovedIndex(a_Index) &&
                        !m_RemovedIndices.IsRemovedIndex(a_Index2) &&
                        !m_RemovedIndices.IsRemovedIndex(a_Index3)) {
                ...
                for (int k = 0; k < a_Triangles.Length; k += 3) {
                    ...
                    if (!m_RemovedIndices.IsRemovedIndex(a_Index4) &&
                        !m_RemovedIndices.IsRemovedIndex(a_Index5) &&
                        !m_RemovedIndices.IsRemovedIndex(a_Index6)) {
            ...
            activeDecalProjector.DecalsMeshUpperBonesIndex = Bones.Count - 1;
        ...
        internal override void RemoveRangesOfVertexAlignedLists(int a_LowerIndex, int a_Count) {
            ...
            OriginalVertices.RemoveRange(a_LowerIndex, a_Count);
            BoneWeights.RemoveRange(a_LowerIndex, a_Count);
        }

        internal override int BoneIndexOffset(SkinnedDecalProjectorBase a_Projector) => a_Projector.DecalsMeshBonesCount;
        ...
        internal override void RemoveBonesAndAdjustBoneWeightIndices(SkinnedDecalProjectorBase a_Projector) {
            ...
            if (decalsMeshBonesCount > 0) {
                Bones.RemoveRange(decalsMeshLowerBonesIndex, decalsMeshBonesCount);
                BindPoses.RemoveRange(decalsMeshLowerBonesIndex, decalsMeshBonesCount);
            }

            for (int i = a_Projector.DecalsMeshLowerVertexIndex; i < BoneWeights.Count; i++) {
                BoneWeight value = BoneWeights[i];
        ...
        int AdjustedIndex(int a_Index, int a_UpperIndex, int a_Offset) {
            int num = a_Index;
        ...
        internal override void AdjustProjectorIndices(SkinnedDecalProjectorBase a_Projector, int a_VertexIndexOffset,
            int a_TriangleIndexOffset, int a_BoneIndexOffset) {
        ...
        void InternalAdd(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Color[] a_Colors, Vector2[] a_UVs,
            Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, int a_BoneIndexOffset, Matrix4x4[] a_BindPoses,
            SkinQuality a_SkinQuality, Vector3 a_ReversedProjectionNormal, float a_CullingDotProduct,
            Matrix4x4 a_WorldToDecalsMatrix, Matrix4x4 a_MeshToDecalsMatrix,
            Matrix4x4 a_MeshToDecalsMatrixInvertedTransposed) {
            if (a_SkinQuality == SkinQuality.Bone1 ||
                a_SkinQuality == SkinQuality.Auto && QualitySettings.skinWeights == SkinWeights.OneBone) {
                AddSkinQuality1(a_Vertices,
                    a_Normals,
                    a_Tangents,
                    a_Colors,
                    a_UVs,
                    ...
                    a_BoneIndexOffset,
                    a_BindPoses,
                    a_ReversedProjectionNormal,
                    a_CullingDotProduct,
                    a_WorldToDecalsMatrix,
                    a_MeshToDecalsMatrix,
                    a_MeshToDecalsMatrixInvertedTransposed);

                return;
            }

            if (a_SkinQuality == SkinQuality.Bone2 ||
                a_SkinQuality == SkinQuality.Auto && QualitySettings.skinWeights == SkinWeights.TwoBones) {
                AddSkinQuality2(a_Vertices,
                    a_Normals,
                    a_Tangents,
                    a_Colors,
                    a_UVs,
        ...
        void AddSkinQuality1(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Color[] a_Colors,
            Vector2[] a_UVs, Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, int a_BoneIndexOffset,
            Matrix4x4[] a_BindPoses, Vector3 a_ReversedProjectionNormal, float a_CullingDotProduct,
            Matrix4x4 a_WorldToDecalsMatrix, Matrix4x4 a_MeshToDecalsMatrix,
            Matrix4x4 a_MeshToDecalsMatrixInvertedTransposed) {
            Color vertexColorTint = m_Decals.VertexColorTint;
            Color color = (1f - ActiveDecalProjector.VertexColorBlending) * ActiveDecalProjector.VertexColor;
            ...
            for (int i = 0; i < a_Vertices.Length; i++) {
                ...
                item.weight2 = 0f;
                item.weight3 = 0f;
                Matrix4x4 matrix4x = Bones[item.boneIndex0].localToWorldMatrix * BindPoses[item.boneIndex0];
                ...
                if (m_Decals.UseVertexColors) {
                    ...
                    Color item5 = vertexColorTint * (color + ActiveDecalProjector.VertexColorBlending * color2);
        ...
        void AddSkinQuality2(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Color[] a_Colors,
            Vector2[] a_UVs, Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, int a_BoneIndexOffset,
            Matrix4x4[] a_BindPoses, Vector3 a_ReversedProjectionNormal, float a_CullingDotProduct,
            Matrix4x4 a_WorldToDecalsMatrix, Matrix4x4 a_MeshToDecalsMatrix,
            Matrix4x4 a_MeshToDecalsMatrixInvertedTransposed) {
            ...
            for (int i = 0; i < a_Vertices.Length; i++) {
                ...
                item.weight3 = 0f;
                Matrix4x4 matrix4x = Bones[item.boneIndex0].localToWorldMatrix * BindPoses[item.boneIndex0];
                Matrix4x4 matrix4x2 = Bones[item.boneIndex1].localToWorldMatrix * BindPoses[item.boneIndex1];
        ...
        void AddSkinQuality4(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Color[] a_Colors,
            Vector2[] a_UVs, Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, int a_BoneIndexOffset,
            Matrix4x4[] a_BindPoses, Vector3 a_ReversedProjectionNormal, float a_CullingDotProduct,
            Matrix4x4 a_WorldToDecalsMatrix, Matrix4x4 a_MeshToDecalsMatrix,
            Matrix4x4 a_MeshToDecalsMatrixInvertedTransposed) {
            ...
            for (int i = 0; i < a_Vertices.Length; i++) {
                ...
                Matrix4x4 matrix4x = Bones[item.boneIndex0].localToWorldMatrix * BindPoses[item.boneIndex0];
                Matrix4x4 matrix4x2 = Bones[item.boneIndex1].localToWorldMatrix * BindPoses[item.boneIndex1];
                Matrix4x4 matrix4x3 = Bones[item.boneIndex2].localToWorldMatrix * BindPoses[item.boneIndex2];
                Matrix4x4 matrix4x4 = Bones[item.boneIndex3].localToWorldMatrix * BindPoses[item.boneIndex3];
                Matrix4x4 matrix4x5 = default;

                for (int j = 0; j < 16; j++) {
                    matrix4x5[j] = matrix4x[j] * item.weight0 +
                                   matrix4x2[j] * item.weight1 +
                                   matrix4x3[j] * item.weight2 +
                                   matrix4x4[j] * item.weight3;
                }
        ...
        internal override void RemoveRange(int a_StartIndex, int a_Count) {
            ...
            OriginalVertices.RemoveRange(a_StartIndex, a_Count);
            BoneWeights.RemoveRange(a_StartIndex, a_Count);
