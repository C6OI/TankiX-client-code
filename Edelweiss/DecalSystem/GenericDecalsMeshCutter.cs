...
namespace Edelweiss.DecalSystem {
    public abstract class GenericDecalsMeshCutter<D, P, DM> where D : GenericDecals<D, P, DM>
        ...
        internal CutEdges m_CutEdges = new();
        internal DM m_DecalsMesh;

        internal CutEdgeDelegate m_GetCutEdgeDelegate;

        internal List<int> m_ObsoleteTriangleIndices = new();

        internal List<RelativeVertexLocation> m_RelativeVertexLocations = new();
        ...
        internal void PlaneDecalsMeshCutter(DM a_DecalsMesh, Plane a_Plane) {
            ...
            for (int j = decalsMeshLowerTriangleIndex; j < count; j += 3) {
                ...
                if (relativeVertexLocation != RelativeVertexLocation.Inside &&
                    relativeVertexLocation2 != RelativeVertexLocation.Inside &&
                    relativeVertexLocation3 != RelativeVertexLocation.Inside) {
                ...
                if (flag) {
                    DecalRay a_Ray = new(vector, vector2 - vector);
                    float a_IntersectionFactorAB = IntersectionFactor(a_Ray, a_Plane);

                    num5 = m_GetCutEdgeDelegate(num,
                        num2,
                        num3,
                        vector,
                        vector2,
                        relativeVertexLocation == RelativeVertexLocation.Inside,
                        a_IntersectionFactorAB);
                }

                if (flag2) {
                    DecalRay a_Ray2 = new(vector, vector3 - vector);
                    float a_IntersectionFactorAB2 = IntersectionFactor(a_Ray2, a_Plane);

                    num6 = m_GetCutEdgeDelegate(num,
                        num2,
                        num4,
                        vector,
                        vector3,
                        relativeVertexLocation == RelativeVertexLocation.Inside,
                        a_IntersectionFactorAB2);
                }

                if (flag3) {
                    DecalRay a_Ray3 = new(vector2, vector3 - vector2);
                    float a_IntersectionFactorAB3 = IntersectionFactor(a_Ray3, a_Plane);

                    num7 = m_GetCutEdgeDelegate(num,
                        num3,
                        num4,
                        vector2,
                        vector3,
                        relativeVertexLocation2 == RelativeVertexLocation.Inside,
                        a_IntersectionFactorAB3);
                }
        ...
        bool Intersect(RelativeVertexLocation a_VertexLocation1, RelativeVertexLocation a_VertexLocation2) =>
            a_VertexLocation1 != 0 && a_VertexLocation2 != 0 && a_VertexLocation1 != a_VertexLocation2;

        float IntersectionFactor(DecalRay a_Ray, Plane a_Plane) {
            float num = Vector3.Dot(a_Plane.normal, a_Ray.direction);
            float num2 = a_Plane.distance - Vector3.Dot(a_Plane.normal, a_Ray.origin);
            return num2 / num;
        }

        Vector3 PlaneOrigin(Plane a_Plane) => a_Plane.distance * a_Plane.normal;

        RelativeVertexLocation
            VertexLocationRelativeToPlane(Vector3 a_PlaneOrigin, Vector3 a_PlaneNormal, Vector3 a_Vertex) {
        ...
        internal int CutEdge(int a_RelativeVertexLocationsOffset, int a_IndexA, int a_IndexB, Vector3 a_VertexA,
            ...
            return m_CreateCutEdgeDelegate(a_RelativeVertexLocationsOffset,
                a_IndexA,
                a_IndexB,
                a_VertexA,
                a_VertexB,
