...
namespace Platform.Kernel.ECS.ClientEntitySystem.Impl {
    public static class SkipInfoBuilder {
        ...
        public static string GetSkipReasonDetails(Handler handler, ArgumentNode fromArgumentNode,
            ArgumentNode toArgumentNode, Optional<JoinType> join) {
            StringBuilder stringBuilder = new();

            stringBuilder.AppendFormat("{0} was skiped, because {1} not found ",
                handler.Name,
                toArgumentNode.argument.ClassInstanceDescription.NodeClass.FullName);
            ...
            if (join.IsPresent() && !(join.Get() is JoinAllType) && fromArgumentNode.entityNodes.Count > 0) {
                ...
                Type componentType = join.Get().ContextComponent.Get();
                groupComponent = (GroupComponent)entity.GetComponent(componentType);
            }
            ...
            foreach (Entity allEntity in Engine.EntityRegistry.GetAllEntities()) {
                int num2 = 0;
                List<Type> list3 = new();
                List<Type> list4 = new();
                list3.Clear();
            ...
            if (entity2 != null) {
                stringBuilder.AppendFormat("\n Best node was {0} , presentComponents {1}, absentComponents {2} ",
                    entity2,
                    EcsToStringUtil.ToString(list),
                    EcsToStringUtil.ToString(list2));
            }
