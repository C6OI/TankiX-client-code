...
namespace Platform.Kernel.ECS.ClientEntitySystem.Impl {
    public class HandlerResolver {
        ...
        public virtual IList<HandlerInvokeData> Resolve(ICollection<Handler> handlers, Event eventInstance,
            ICollection<Entity> contextEntities) {
            if (handlers.Count == 0) {
            ...
            while (enumerator.MoveNext()) {
                ...
                HandlerInvokeData invokeData =
                    ((EngineServiceImpl)EngineService).HandlerContextDataStorage.GetInvokeData(current,
                        eventInstance.GetType(),
                        contextEntities);

                if (invokeData.Reuse(eventInstance) ||
                    UpdateInvokeData(invokeData, current, eventInstance, contextEntities)) {
        ...
        protected virtual bool UpdateInvokeData(HandlerInvokeData invokeData, Handler handler, Event eventInstance,
            ICollection<Entity> contextEntities) {
