...
namespace Platform.Kernel.ECS.ClientEntitySystem.Impl {
    public class SkipLog {
        ...
        ICollection<Entity> PrintReasonForHandlerArgument(HandlerArgument handlerArgument, ICollection<Entity> leftEntities,
            ICollection<LogPart> parts) {
            if (handlerArgument.Context) {
                ...
                if (handlerArgument.JoinType.Get().ContextComponent.IsPresent()) {
                    ...
                    foreach (Entity item in collection) {
                        foreach (Entity groupMember in ((GroupComponent)item.GetComponent(groupComponent)).GetGroupMembers(
                                     handlerArgument.NodeDescription)) {
        ...
        static void NewLine(StringBuilder o) => o.Append("\n\t");
    }
}