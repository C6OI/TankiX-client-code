...
namespace Platform.System.Data.Statics.ClientConfigurator.API {
    public class TarImporter {
        ...
        public T ImportAll<T>(Stream inputStream) where T : ConfigTreeNode, new() =>
            Import<T>(inputStream, AnyProfile.Instance);
        ...
        T ReadToConfigTree<T>(Stream inputStream, ConfigurationProfile configurationProfile)
            where T : ConfigTreeNode, new() {
            T result = new();
        ...
        string GetDirectoryName(string filePath) => Path.GetDirectoryName(filePath).Trim(Path.DirectorySeparatorChar)
