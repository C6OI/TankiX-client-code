...
namespace Platform.Library.ClientResources.API {
    public class AssetStorageComponent : Component {
        ...
        public void Remove(string guid, ResourceStoreLevel level) {
            switch (level) {
                case ResourceStoreLevel.STATIC:
                    ManagedReferencies.Remove(guid);
                    StaticReferencies.Remove(guid);
                    break;

                case ResourceStoreLevel.MANAGED:
                    ManagedReferencies.Remove(guid);
                    break;
            }
        }

        public void Put(string guid, Object asset, ResourceStoreLevel level) {
            switch (level) {
                case ResourceStoreLevel.STATIC:
                    if (!StaticReferencies.ContainsKey(guid)) {
                ...
                case ResourceStoreLevel.MANAGED:
                    if (!ManagedReferencies.ContainsKey(guid)) {
                        ManagedReferencies.Add(guid,
                            new ResourceStorageEntry {
                                Asset = asset,
