        ...
        public string ProfileElement {
            get {
                ...
                if (string.IsNullOrEmpty(language)) {
                    ...
                    try {
                        defaultLocaleConfiguration = configurationService.GetConfig(ConfigPath.DEFAULT_LOCALE)
                            .ConvertTo<DefaultLocaleConfiguration>();
                    } catch (Exception ex) {
                        LoggerProvider.GetLogger(this).WarnFormat("Unable to read default lcoale  {0}. {1}", ex.Message, ex);
                    }

                    if (defaultLocaleConfiguration != null &&
                        !string.IsNullOrEmpty(defaultLocaleConfiguration.DefaultLocale)) {
