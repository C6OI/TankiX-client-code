...
namespace Platform.Library.ClientDataStructures.Impl.Cache {
    public class CacheMultisizeArrayImpl<T> : AbstractCache, CacheMultisizeArray<T> {
        ...
        public void Dispose() => elementsCountToArrayCache.Clear();

        public void FreeAll() => elementsCountToArrayCache.ForEach(delegate(KeyValuePair<int, CacheImpl<T[]>> kv) {
            kv.Value.FreeAll();
        });
        ...
        public void Dispose(int length) => elementsCountToArrayCache.Remove(length);
        ...
        public void FreeObjectInstance(object item) => throw new NotImplementedException();
