...
namespace SharpCompress.Archive {
    public abstract class AbstractWritableArchive<TEntry, TVolume> : AbstractArchive<TEntry, TVolume>
        ...
        IEnumerable<TEntry> OldEntries => base.Entries.Where(x => !removedEntries.Contains(x));
        ...
        public void AddEntry(string filePath, Stream source, long size = 0, DateTime? modified = null) {
            CheckWritable();
            newEntries.Add(CreateEntry(filePath, source, size, modified, false));
            RebuildModifiedCollection();
        }

        public void AddEntry(string filePath, Stream source, bool closeStream, long size = 0, DateTime? modified = null) {
            CheckWritable();
        ...
        protected abstract void SaveTo(Stream stream, CompressionInfo compressionType, IEnumerable<TEntry> oldEntries,
