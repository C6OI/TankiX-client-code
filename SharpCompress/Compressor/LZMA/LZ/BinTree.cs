...
namespace SharpCompress.Compressor.LZMA.LZ {
    class BinTree : InWindow {
        const uint kHash2Size = 1024u;

        const uint kHash3Size = 65536u;

        const uint kBT2HashSize = 65536u;

        const uint kStartMaxLen = 1u;

        const uint kHash3Offset = 1024u;

        const uint kEmptyHashValue = 0u;

        const uint kMaxValForNormalize = 2147483647u;
        ...
        uint kFixHashSize = 66560u;

        uint kMinMatchCheck = 4u;
        ...
        public uint GetMatches(uint[] distances) {
            ...
            if (kNumHashDirectBytes != 0 &&
                num10 > num3 &&
                _bufferBase[_bufferOffset + num10 + kNumHashDirectBytes] != _bufferBase[num4 + kNumHashDirectBytes]) {
            ...
            while (true) {
                ...
                uint num16 = (num15 > _cyclicBufferPos ? _cyclicBufferPos - num15 + _cyclicBufferSize
        ...
        public void Skip(uint num) {
            do {
                ...
                while (true) {
                    ...
                    uint num13 = (num12 > _cyclicBufferPos ? _cyclicBufferPos - num12 + _cyclicBufferSize
