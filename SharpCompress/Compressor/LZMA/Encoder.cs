...
namespace SharpCompress.Compressor.LZMA {
    class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties {
        ...
        uint GetRepLen1Price(Base.State state, uint posState) =>
            _isRepG0[state.Index].GetPrice0() + _isRep0Long[(state.Index << 4) + posState].GetPrice0();
        ...
        uint GetPosLenPrice(uint pos, uint len, uint posState) {
            ...
            uint num = pos >= 128 ? _posSlotPrices[(lenToPosState << 6) + GetPosSlot2(pos)] + _alignPrices[pos & 0xF]
        ...
        uint GetOptimum(uint position, out uint backRes) {
            ...
            while (true) {
                ...
                uint num20 = price2 +
                             _isMatch[(state.Index << 4) + num5].GetPrice0() +
                ...
                if (!flag && indexByte2 != indexByte) {
                    ...
                    if (matchLen >= 2) {
                        ...
                        uint num23 = num20 +
                                     _isMatch[(state2.Index << 4) + num22].GetPrice1() +
                ...
                for (uint num27 = 0u; num27 < 4; num27++) {
                    ...
                    if (matchLen2 >= 2) {
                        ...
                        uint num32 = num6 +
                                     GetRepPrice(num27, num28, state, num5) +
                                     _isMatch[(state3.Index << 4) + num31].GetPrice0() +
                                     _literalEncoder
                                         .GetSubCoder(position + num28, _matchFinder.GetIndexByte((int)(num28 - 1 - 1)))
                                         .GetPrice(true,
                    ...
                    if (num38 == _matchDistances[num37]) {
                        if (num38 < val) {
                            ...
                            if (matchLen3 >= 2) {
                                ...
                                uint num42 = num40 +
                                             _isMatch[(state4.Index << 4) + num41].GetPrice0() +
                                             _literalEncoder
                                                 .GetSubCoder(position + num38,
                                                     _matchFinder.GetIndexByte((int)(num38 - 1 - 1))).GetPrice(true,
        ...
        class LiteralEncoder {
            ...
            public Encoder2 GetSubCoder(uint pos, byte prevByte) =>
                m_Coders[(int)((pos & m_PosMask) << m_NumPrevBits) + (prevByte >> 8 - m_NumPrevBits)];
