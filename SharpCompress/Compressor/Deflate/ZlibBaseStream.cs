    ...
    class ZlibBaseStream : Stream {
        ...
        void finish() {
            ...
            if (_streamMode == StreamMode.Writer) {
                ...
                if (crc32Result2 != num4) {
                    throw new ZlibException(string.Format("Bad CRC32 in GZIP stream. (actual({0:X8})!=expected({1:X8}))",
                        crc32Result2,
                        num4));
                }

                if (num6 != num5) {
                    throw new ZlibException(string.Format("Bad size in GZIP stream. (actual({0})!=expected({1}))",
                        num6,
                        num5));
                }
            }
        ...
        public override int Read(byte[] buffer, int offset, int count) {
            ...
            do {
                ...
                if (num != 0 && num != 1) {
                    throw new ZlibException(string.Format("{0}flating:  rc={1}  msg={2}",
                        !_wantCompress ? "in" : "de",
                        num,
                        _z.Message));
                }
            } while ((!nomoreinput && num != 1 || _z.AvailableBytesOut != count) &&
